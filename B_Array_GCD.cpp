...
void func()
{
    ...
    range(i, n)
    {
        auto tmp = toprimes(vec[i]);
        auto tmp = toprimes(vec[i] + 1);
        auto tmp = toprimes(vec[i] - 1);
        foreach (i, tmp)
        {
            store[i]++;
    ...
    ll maxivalue = 0;
    foreach (i, store)
        maxivalue = max(maxivalue, i.second);
    ll towvalue = n - maxivalue;

    ll ans = min(towvalue * a, (towvalue - 1) * a + b);
    print(ans);
}
int main()
{

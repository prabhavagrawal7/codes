...
void func()
{
    ...
    if (k == 2)
    {
        range(i, n)
        {
            range(j, i + 1, n)
            {
                ll val = abs(vec[i] - vec[j]);
                ll nit = upper_bound(all(vec), val) - vec.begin();
                ll pit = upper_bound(all(vec), val) - vec.begin() - 1;
                if (nit < n)
                    ans = min(ans, abs(vec[nit] - val));
                if (pit >= 0)
                    ans = min(ans, abs(vec[pit] - val));
            }
        }
    }
    ans = min(ans, *min_element(all(vec)));
    print(ans);
}
int main()
{

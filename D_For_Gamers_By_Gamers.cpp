...
void func()
{
    ...
    range(i, n){ 
        newint(a1, a2, a3); 
        nvec[a1] = max(nvec[a1], a2*a3); 
    }
    // mii nvec; 
    range(i, nvec.size())
    {
        if(nvec[i] == -1) continue; 
        for(ll j = 1; i*j <= c; j++)
        {
            nvec[i*j] = max(nvec[i*j], nvec[i]*j); 
        }
    }
    ll bstp = 0; 
    range(i, nvec.size())
    {
        if(nvec[i] == -1) continue;
        nvec[i] = max(nvec[i], bstp); 
        bstp = nvec[i]; 
    }
    V<pii> pvec; 
    pvec.push_back({0LL, 0LL}); 

    range(i, nvec.size())
    {
        if(nvec[i] != -1) {
            pvec.push_back({i, nvec[i]});
        }
    }
    foreach(i, pvec){swap(i.first, i.second);}
    // h*a , coins
    newint(m); 
    range(m)
    {
        newint(hm, am); 
        ll ev = hm*am; 
        if(ev >= pvec[pvec.size()-1].first) {printl(-1); continue; }
        ll l = 1, r = pvec.size()-1, mid; 
        while(l <= r)
        {
            mid = (l+r)/2;
            if(pvec[mid].first > ev && pvec[mid-1].first <= ev)
            {
                break; 
            }
            else if(pvec[mid].first > ev) {
                r = mid - 1;  
            }
            else l = mid + 1; 
        }
        printl(pvec[mid].second); 
    }

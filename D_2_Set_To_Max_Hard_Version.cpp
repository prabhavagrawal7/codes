...
public:
    ...
    {
        ll mini, maxi, lazy;
        Node(ll mini, ll maxi, ll lazy) : mini(mini), maxi(maxi), lazy(lazy) {}
        Node() : mini(INT_INF), maxi(0), lazy(-1) {}
    };
    V<Node> tree;
    ll n;
    seg(ll n)
    {
        this->n = n;
        tree.assign(1LL << (clz(n) + 2), Node()); // mini, maxi, lazy
    }
    void push(ll node, ll start, ll end)
    {
        ...
        if (tree[node].lazy == -1)
            return;
        tree[lnode] = tree[rnode] = tree[node];
        tree[node].lazy = -1;
    }
    Node opr(Node a, Node b)
    {
        // critical
        return Node(min(a.mini, b.mini), max(a.maxi, b.maxi), -1);
    }
    ...
    Node rfind(ll l, ll r, ll val, ll node, ll start, ll end)
    {
        if (r < start || end < l)
            return Node();
        else if (l <= start && end <= r)
        ...
        push(node, start, end);
        return opr(rfind(l, r, val, lnode, start, mid), rfind(l, r, val, rnode, mid + 1, end));
    }
};
void func()
{

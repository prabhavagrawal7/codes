...
for (int i = 0; i < v.size(); ++i){os << v[i];if (i != v.size() - 1)os << " ";}return os;}
...
public:
    ...
    template <typename T, typename = typename enable_if<is_arithmetic<T>::value, T>::type>
    inline bool operator>=(T a){ return (this->val >= a); }
    friend istream &operator>>(istream &cc, rll &a){ cc >> a.val; return cc; }
};
rll power(rll x, rll y){rll res = 1;while (y){if (y & 1LL)res = (res * x);y = y / 2;x = (x * x);}return res;}
inline ll rs(ll n) { return (n = n % mod) >= 0 ? n : n + mod; }
ll power(ll x, ll y){ll res=1;while(y){if(y&1LL)res=(res*x)%mod;y>>=1;x=(x*x)%mod;}return res%mod;}

/* -------------------------------------------------------------------------------------------------------------------------------------------------------- */
vi findoutedge(vvi &g)
{
    vi vis(g.size()); 
    range(i, 1, g.size())
    {
    ...
    vi ans; 
    range(i, 1, vis.size()) if(vis[i] == 0) ans.push_back(i); 
    return ans; 
}
ll dfs(ll x, vi &depths, vvi &g)
{
    bool noloop = false; 
    foreach(i, g[x])
    {px

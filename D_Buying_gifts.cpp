...
void func()
{
    ...
    reverse(all(vec)); 
    MS<ll> s;
    range(i, vec.size()) s.insert(vec[i].second);
    ll ans = INF;
    ll m = -1;
    range(i, n)
    {
        ll big = vec[i].first;
        s.erase(s.find(vec[i].second));
        auto c = s.lower_bound(big);
        if(m != -1) ans = min(ans, abs(m - big));
        if (c != s.begin()) ans = min(ans, abs(max(m, *prev(c)) - big));
        if (c != s.end()) ans = min(ans, abs(max(m, *c) - big));

...
int minimumDeletions(string word, int k)
{
    ...
    {
        ll minf = i, maxf = i + k;
        // make sure every frequency is between minf and maxf
        // if not, we need to delete some characters or delete the whole character
        int tempans = 0;
        for (int j = 0; j < 26; j++)
        {
            if (freq[j] < minf)
...
int main()
{
    ...
    range(t)
    {
        newstring(str);
        newint(k);
        cout << minimumDeletions(str, k) << endl;
    }
}

...
void func()
{
    ...
    struct node
    {
        ...
        ll prefix_sum;
        ll storage_sum;
        node(ll a, ll b, ll c) : value(a), prefix_sum(b), storage_sum(c){};
    };
    vector<node> store_values = {{-INT_INF, 0, 0}};
    vector<int> store_min;
    range(i, n)
    {
        ll new_storage_sum = 0;
        while (store_values.back().value > vec[i])
        {
            new_storage_sum += store_values.back().storage_sum;
            new_storage_sum = rs(new_storage_sum);
            store_values.pop_back();
        }
        dp[i] += new_storage_sum;
        dp[i] += store_values.back().prefix_sum;
        if (store_values.size() == 1)
            dp[i]++;
        dp[i] = rs(dp[i]);

        ll new_value = vec[i];
        ll new_prefix_sum = rs(store_values.back().prefix_sum + dp[i]); 
        ll storage_sum = rs(new_storage_sum + dp[i]); 
        store_values.push_back({new_value, new_prefix_sum, storage_sum});
    }
    ll value = INT_INF;
    ll ans = 0;
    range(i, n - 1, -1, -1)

...
vector<tuple<int, int, int>> findTriplets2(int n)
{
    auto f = generate(n);
    ...
    range(i, f.size())
    {
        range(j, f.size())
        {
            range(k, f.size())
            {
                ll x = f[i][0] + f[j][0] * 10 + f[k][0] * 100;
                ll y = f[i][1] + f[j][1] * 10 + f[k][1] * 100;
                ll z = f[i][2] + f[j][2] * 10 + f[k][2] * 100;
                triplets.push_back(make_tuple(x, y, z));
            }
        }
    }
    return triplets;
...
int main()
{
    ...
    // Test the function
    int n = 777;
    vector<tuple<int, int, int>> result = findTriplets2(7);

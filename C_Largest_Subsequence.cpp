...
void func()
{
    ...
    string str;
    cin >> str;
    vector<pair<char, ll>> subsequence;
    ...
    range(i, str.size() - 1, -1, -1)
    {
        if (lastchar > str[i])
            continue;
        subsequence.push_back({str[i], i});
        lastchar = str[i];
    }

    ll ans = subsequence.size(), m = subsequence.size();
    string newstr = str;
    for (int i = 0; i < subsequence.size() / 2; i++)
    {
        ll idx1 = subsequence[i].second;
        ll idx2 = subsequence[m - i - 1].second;
        swap(newstr[idx1], newstr[idx2]);
    }
    if (is_sorted(all(newstr)))
    {
        while (subsequence.size() > 1 &&
               subsequence[subsequence.size() - 1].first == subsequence[subsequence.size() - 2].first)
            subsequence.pop_back();
        give((ll)subsequence.size() - 1);
    }
    else
        give(-1);
}

...
ll code(ll x, vvi &g, ll parent, bool shouldtake, V<pii> &p, V<pii> &dp)
{
    if (shouldtake == false && dp[x].first != -1)
        return dp[x].first;

    if (shouldtake == true && dp[x].second != -1)
        return dp[x].second;

    ll ans = 0;
    ll comp = shouldtake ? p[x].second : p[x].first;
    foreach (i, g[x])
    {
        if (parent == i)
            continue;
        ll ans1 = abs(comp - p[i].second) + code(i, g, x, true, p, dp);
        ll ans2 = abs(comp - p[i].first) + code(i, g, x, false, p, dp);
        ans += max(ans1, ans2);
    }
    if (shouldtake == false)
        return dp[x].first = ans;
    else
        return dp[x].second = ans;
}
...
void func()
{
    ...
    V<pii> dp(n + 1, {-1, -1});
    ll ans = code(1, g, -1, false, p, dp);
    ans = max(ans, code(1, g, -1, true, p, dp));
    print(ans);
}
int main()
{

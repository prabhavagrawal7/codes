...
ll code(string &str, ll start, map<char, vi> &hsh, vi &dp)
{
    if(start == str.size()) {return 0; }
    if(dp[start] != INT_INF) {
        return dp[start]; 
    }
    if(start+1 < str.size() && str[start] == str[start+1])
    {
        ll ans1 = code(str, start+2, hsh, dp); 
        ll ans2 = 1 + code(str, start+1, hsh, dp); 
        return dp[start] = min(ans1, ans2);
    }
    else {
        ll ans1 = INT_INF; 
        auto x = upper_bound(hsh[str[start]].begin(), hsh[str[start]].end(), start); 
        if(x != hsh[str[start]].end())
        {
            ans1 = *x - start - 1 + code(str, *x + 1, hsh, dp); 
        }
        ll ans = 1 + code(str, start + 1, hsh, dp); 
        return dp[start] = min(ans1, ans); 
    }
}
...
void func()
{
    ...
    map<char, vi> hsh; 
    range(i, str.size()) {
        hsh[str[i]].push_back(i); 
    }
    vi dp(str.size() + 1, INT_INF); 
    ll u = code(str, 0, hsh, dp); 
    print(u); 
}
int main()
{

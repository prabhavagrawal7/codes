...
#endif
/* ----------------------------------------------------------------------------------------------*/
ll rtnf(string str, string str1)
{
    ll n = str.size();
    if (str == str1)
    {
    ...
    ll ans = INT_INF;
    if (changes % 2 == 0) // changes are even
        ans = min(ans, changes * 2);
    else 
        ans = min(ans, changes * 2 - 1);
    return ans;
}
ll revrtnf(string str, string str1)
{
    reverse(all(str1));
    ll n = str.size();
    ll changes = 0;
    ...
    ll ans = INT_INF;
    if (changes % 2 == 1) // changes are even
        ans = min(ans, changes * 2);
    else
        ans = min(ans, changes * 2 - 1);
    return ans;
...
void func()
{
    ...
    ll ans = rtnf(str, str1);
    ans = min(ans, revrtnf(str, str1));
    print(ans);
}
int main()
{

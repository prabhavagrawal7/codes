...
#define lnode (node * 2 + 1)
#define rnode (node * 2 + 2)
#define neutral_element store()
    struct store
    {
        vi pref;
        ll val;
        store() : pref(vi(41, 0)), val(0){};
    };
    V<store> tree;
    ll n;
    seg(ll n)
    {
        this->n = n;
        assert((1LL << (clz(n) + 2)) < 1e6); 
        tree.assign(1LL << (clz(n) + 2), store());
    }
    store opr(store a, store b)
    {
        ...
        c.val = a.val + b.val;
        range(i, c.pref.size())
            c.pref[i] = a.pref[i] + b.pref[i];
        range(i, 1, b.pref.size())
        {
            ll cnt = b.pref[i - 1] - b.pref[i];
            ll cnt2 = a.pref[i];
            c.val += cnt * cnt2;
        }
        return c;
    ...
    store rfind(ll l, ll r, ll node, ll start, ll end)
    {
        if (r < start || end < l)
            return neutral_element;
        else if (l <= start && end <= r)
            return tree[node];
        return opr(rfind(l, r, lnode, start, mid),
...
void func()
{
    ...
    range(q)
    {
        ...
        newint(type, a, b);
        if (type == 1)
            print(s.rfind(a - 1, b - 1, 0, 0, n - 1).val);
        else if (type == 2)
            s.update(a - 1, b, 0, 0, n - 1);
    }
}
int main()
{

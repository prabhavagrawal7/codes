...
#endif
/* ----------------------------------------------------------------------------------------------*/
ll rec(ll n, ll prev, vi &vec, vi &hot, vi &cold)
{
    if (n == vec.size())
        return 0;
    ...
    if (vec[n] == vec[n - 1])
    {
        ans = min(ans, rec(n + 1, prev, vec, hot, cold) + hot[vec[n]]);
    }
    if (vec[n] == vec[prev])
    {
        ans = min(ans, rec(n + 1, n - 1, vec, hot, cold) + hot[vec[n]]);
    }

    ans = min(ans, rec(n + 1, prev, vec, hot, cold) + cold[vec[n]]);
    ans = min(ans, rec(n + 1, n - 1, vec, hot, cold) + cold[vec[n]]);
    return ans;
}
void func()
{
    newint(n, k);
    vi vec = inputvec(n);
    vi hot = inputvec(k + 1, 1);
    vi cold = inputvec(k + 1, 1);
    cout << rec(1, 0, vec, hot, cold) << endl;
}
int main()
{
    FAST;

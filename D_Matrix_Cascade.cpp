...
void func()
{
    ...
    V<vvi> store(n + 1, vvi(n + 1, {0, 0, 0}));
    ll count = 0;
    range(i, n)
    {
        range(j, n)
        {
            bool exp = ((vstr[i][j] - '0') ^ store[i][j][0] ^ store[i][j][1] ^ store[i][j][2]);
            if (exp != 0)
            {
                count++;
                store[i][j][0] = !store[i][j][0];
                store[i][j][1] = !store[i][j][1];
                store[i][j][2] = !store[i][j][2];
            }
            if (store[i][j][0] && btn(0, i + 1, n - 1) && btn(0, j - 1, n - 1))
                store[i + 1][j - 1][0] = !store[i + 1][j - 1][0];

            if (store[i][j][1] && btn(0, i + 1, n - 1))
            {
                store[i + 1][j][0] = !store[i + 1][j][0];
                store[i + 1][j][1] = !store[i + 1][j][1];
                store[i + 1][j][2] = !store[i + 1][j][2];
            }

            if (store[i][j][2] && btn(0, i + 1, n - 1) && btn(0, j + 1, n - 1))
                store[i + 1][j + 1][2] = !store[i + 1][j + 1][2];
        }
    }
    print(count);
}

...
void func()
{
    ...
    foreach (i, freq)
    {
        elements.pb(i.second);
    }
    sort(all(elements), greater<ll>());
    ll time = elements.size();
    range(i, elements.size())
    {
        elements[i] = elements[i] - elements.size() + i; 
    }
    MS<ll> el(all(elements)), newel; 
    while(el.size())
    {
        while(el.size() && *el.begin() <= 0) el.erase(el.begin()); 
        foreach(i, el)
        {
            newel.insert(i - 1); 
        }

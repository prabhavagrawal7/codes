...
void func()
{
    ...
    vvi ans1(32), ans2(32);
    ll maxidx = max_element(all(vec)) - vec.begin();
    ll minidx = min_element(all(vec)) - vec.begin();
    if (vec[maxidx] > 0)
    {
        vvi().swap(ans1);
        vi cvec = vec;
        while (cvec[maxidx] + cvec[minidx] < 0)
        {
            ans1.push_back({maxidx + 1, maxidx + 1});
            cvec[maxidx] += cvec[maxidx];
        }
        range(i, n)
        {
            if (cvec[i] >= 0)
                continue;
            cvec[i] += cvec[maxidx];
            ans1.push_back({i + 1, maxidx + 1});
        }

        range(i, n - 1)
        {
            cvec[i + 1] += cvec[i];
            ans1.push_back({i + 2, i + 1});
        }
    }
    if (vec[minidx] < 0)
    {
        ...
        while (cvec[maxidx] + cvec[minidx] > 0)
        {
            ans2.push_back({minidx + 1, minidx + 1});
            cvec[minidx] += cvec[minidx];
        }
        range(i, n)
        {
            if (cvec[i] <= 0)
                continue;
            cvec[i] += cvec[minidx];
            ans2.push_back({i + 1, minidx + 1});
        }
        range(i, n - 1, 0, -1)
        {
            cvec[i - 1] += cvec[i];
            ans2.push_back({i - 1 + 1, i + 1});
        }
    }

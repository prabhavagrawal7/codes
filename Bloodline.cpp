...
ll rec(ll win)
{
    if (popcount(win) <= 25 && store.count(win))
        return store[win];
    range(i, 10, -1, -1)
    {
        if (win - (1 << i) >= 0 && (!rec(win - (1 << i))) == 0)
            return popcount(win) <= 25 ? store[win] = 0 : 0;
    }
    return popcount(win) <= 20 ? store[win] = 1 : 1;
}
void func()
{
    newint(n, k);
    // represent n in the form of k base number system
    vi ans;
    while (n)
    {
        ans.pb((n % k) % 2);

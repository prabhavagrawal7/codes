...
void func()
{
    ...
    range(i, n - 1, -1, -1)
    {
        if (vec[i] == 0)
        {
            if (sortbyfreq.size() == 0)
                ans += 1;
            else
            {
                auto x = prev(sortbyfreq.end());
                ans += x->first;
            }
            sortbyfreq.clear();
            freq.clear();
        ...
        {
            tsum += vec[i];
            sortbyfreq[freq[tsum]].erase(tsum);
            if (sortbyfreq[freq[tsum]].size() == 0)
            {
                sortbyfreq.erase(freq[tsum]);
            }
            freq[tsum]++;
            sortbyfreq[freq[tsum]].insert(tsum);
        }
    }
    print(ans - 1);
}

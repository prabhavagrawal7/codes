...
{
    vector<tries *> next;
    tries() : next(vector<tries *>(10, nullptr))
    {
    }
};
void insert(vi vec, tries *head)
{
    V<pii> pvec;
    range(i, vec.size())
    {
        pvec.push_back({vec[i], i});
    }
    sort(all(pvec));
    for (int i = 0; i < vec.size(); i++)
    {
        ll pos = pvec[i].second;
        if (head->next[pos] == nullptr)
            head->next[pos] = new tries();
        head = head->next[pos];
    }
}
ll check(vi vec, tries *head)
{
    ll ans = 0; 
    for (int i = 0; i < vec.size(); i++)
    {
...
void erase(tries* head)
{
    for(int i = 0; i < head->next.size(); i++)
    {
        if(head->next[i] != nullptr) erase(head->next[i]); 
    }
    delete head; 
}
void func()

...
void func()
{
    ...
    sort(all(indexed), [](P<vs, ll> &x, P<vs, ll> &y)
         {
        // if(x.first == y.first) 
            // return x.second < y.second; 
        // else 
        return eql(x.first, y.first); });

        // range(i, indexed.size())
        // {
        //     range(j, i + 1, indexed.size())
        //     {
        //         if (!eql(indexed[i].first, indexed[j].first))
        //             swap(indexed[i], indexed[j]);
        //     }
        // }
        print(indexed[0].second + 1);

    vvi report;
    range(i, indexed.size() - 1)
    {
        report += rpt(indexed[i], indexed[i + 1]);
    }
    print(report.size());
    foreach (i, report)
        print(i);

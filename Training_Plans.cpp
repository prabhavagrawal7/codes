...
void func()
{
    ...
    while(svec.size())
    {
        auto x = prev(svec.end()); 
        sum += *x; 
        level++; 
        if(mv.count(level)) {
            foreach(i, mv[level]) svec.insert(i); 
        }
        maxsumval = max(maxsumval, TLD(sum)/TLD(level)); 
        svec.erase(x); 
    }
    cout << fixed << setprecision(6) << maxsumval << endl;
    // print(maxsumval); 

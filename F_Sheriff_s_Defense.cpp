...
#endif
/* ----------------------------------------------------------------------------------------------*/
ll dfs(ll n, ll par, bool par_taken, vvi &g, vi &gold, ll c, vvi &dp)
{
    if (dp[n][par_taken] != -INF)
        return dp[n][par_taken];

    ll notTakenAns = 0, takenAns = gold[n];
    if (par_taken)
        takenAns -= c;
    foreach (i, g[n])
    {
        if (i == par)
            continue;
        notTakenAns += dfs(i, n, 0, g, gold, c, dp);
    }
    ...
    foreach (i, g[n])
    {
        if (i == par)
            continue;
        gold[i] -= c;
        takenAns += dfs(i, n, 1, g, gold, c, dp);
        gold[i] += c;
    }

    return dp[n][par_taken] = max(notTakenAns, takenAns);
}
void func()
{
    newint(n, c);
    vi gold = inputvec(n + 1, 1);
    vvi g(n + 1);
    range(i, n - 1)
    {
        newint(u, v);

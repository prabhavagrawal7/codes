...
void func()
{
    ...
    range(i, n - 1, -1, -1)
    {
        // even index se add kar sakte
        // odd index se add nahi kar sakte
        if (i % 2 == 0 && vec[i] > 0)
        {
            ans += vec[i];
            foreach (i, traversed)
        ...
        else if (i % 2 == 1 && vec[i] > 0)
        {
            traversed.push_back({vec[i], i});
        }
    }
    ll extraans = 0;
    if (traversed.size())
    {
        ll totaltraversed = 0;
        foreach (i, traversed)
            totaltraversed += i.first;
        extraans = max(extraans, totaltraversed + vec[0]);
        
        ll bali = max(vec[1], 0LL); 
        extraans = max(extraans, totaltraversed - bali); 
    }
    print(ans + extraans); 
}
int main()

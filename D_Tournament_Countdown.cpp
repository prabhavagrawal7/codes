...
void func()
{
    ...
    vi vec;
    vi tempvec;
    range(i, 1LL << n) vec.push_back(i + 1);

    while (vec.size() != 1)
    {
        if (vec.size() == 2)
        {
            give("!", query(vec[0], vec[1]));
        }
        range(i, 0, vec.size(), 4)
        {
            ll res = query(vec[i], vec[i + 2]);
            if (res == 0)
                tempvec.pb(query(vec[i + 1], vec[i + 3]));
            else if (res == vec[i])
                tempvec.pb(query(vec[i], vec[i + 3]));
            else if (res == vec[i + 2])
                tempvec.pb(query(vec[i + 1], vec[i + 2]));
        }
        vec = move(tempvec);
    }
    give("!", vec[0]);

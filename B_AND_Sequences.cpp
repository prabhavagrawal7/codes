...
{
    if(n == 0) return 1; 
    else return rs(n * factorial(n-1)); 
}
void func()
{
    newint(n); 
    vi vec = inputvec(n); 
    ll sm = accumulate(all(vec), INT_INF*1LL, [](ll a, ll b) { return a & b; }); 
    sort(all(vec)); 
    if(vec[0] != sm) {give(0); }
    ll u = upper_bound(all(vec), vec[0]) - lower_bound(all(vec), vec[0]); 
    ll ans = rs(rs(u * (u - 1)) * factorial(n - 2)); 
    print(ans); 
}
int main()
{

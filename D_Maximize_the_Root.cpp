...
#endif
/* ----------------------------------------------------------------------------------------------*/
bool check(ll n, vvi &g, ll par, vi &weights, ll required, ll removal)
{
    removal = min(removal, (ll)1e12);
    ll newremoval = max(0LL, required + removal - weights[n]);
    bool inloop = false;
    foreach (i, g[n])
    {
        if (i == par)
            continue;
        inloop = true;
        if (check(i, g, n, weights, required, newremoval + removal) == false)
        {
            return false;
        }
    }
    if (inloop == true)
    {
        weights[n] = weights[n] - removal;
        weights[n] = max(weights[n], required);
    }
    if (inloop == false && weights[n] - removal < required)
    {   
        return false;
    }
    return true;
...
void func()
{
    ...
    while (r - l > 1)
    {
        ...
        foreach (i, g[1])
        {
            flag &= check(i, g, 1, nweights, mid, 0);
        }
        if (flag)
            l = mid;
        else
    ...
    print(weights[1] + l);
    // check(1, g, 0, weights, l, 0);
    // ll mini = INT_INF;
    // foreach (i, g[1])
    // {
    //     mini = min(mini, weights[i]);
    // }
    // print(mini + max(weights[0], l));
}
int main()
{
    FAST;

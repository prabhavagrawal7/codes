...
ll func(vi &vec, ll start, ll k)
{
    ...
    ll nextitr = upper_bound(vec.begin(), vec.end(), vec[start] + 5) - vec.begin();
    ll ans1 = func(vec, nextitr, k - 1);
    ll ans2 = func(vec, start + 1, k);
    return dp[start][k] = max(nextitr - start + ans1, ans2);
}

...
#endif
/* ----------------------------------------------------------------------------------------------*/
bool is_unhappy(ll x, const vi &a, ll total_wealth, ll max_wealth, ll n)
{
    ll new_total_wealth = total_wealth + x;
    ll unhappy_count = 0;
    range(i, n - 1)
    {
        if (a[i] * 2 * n < new_total_wealth)
            unhappy_count++;
    }
...
void func()
{
    ...
    vi a = inputvec(n);
    sort(all(a));
    ll total_wealth = accumulate(all(a), 0LL);
    ll max_wealth = *max_element(all(a));

    ll low = -1, high = 1e12, answer = -1;
    while (high - low > 1)
    {
        ll mid = (low + high) / 2;
        if (is_unhappy(mid, a, total_wealth, max_wealth, n))
        {
            high = mid;
        }
        else
    ...
    // print(is_unhappy(0, a, total_wealth, max_wealth, n));
    if (high == 1e12)
        give(-1);
    print(max(0LL, high));
}

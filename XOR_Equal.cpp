...
ll func()
{
    ...
    // maxf contains max frequency
    // now checking no of operations required

    vi pvec;
    for (auto i : freq1)
    {
        if (i.second == maxf)
            pvec.push_back(i.first);
    }

    ll operations = INT_MAX;
    for (auto i : pvec)
    {
        operations = min(operations, min(freq[i], freq[k ^ i]));
    }
    cout << maxf << " " << operations << endl;
    return 0;
}
int main()
{

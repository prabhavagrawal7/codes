...
void func()
{
    ...
    newstring(str);
    vi vec;
    range(i, str.size()) vec.push_back(str[i] - 'a');
    vi checkvec(1 << 20, -1);
    range(i, k)
    {
        newstring(cstr);
    ...
    range(i, checkvec.size() - 1, -1, -1)
    {
        if (checkvec[i] == -1)
            continue;
        // take msb and reduce it
        ll x = 0;
        while ((1 << (x + 1)) <= i)
            x += 1;
        checkvec[i ^ (1 << x)] = checkvec[i];
    }
    ...
    range(i, n)
    {
        ll check = 0;
        ll J = 0;
        range(j, i, min(i + 20, n))
        {
            ll tempcheck = (check | (1LL << vec[j]));
            if (checkvec[tempcheck] == -1)
                break;
            check = tempcheck;
            J = j;
        }
        far = max(far, J);
        if (far == J)
            range(i, far + 1) if (vis[i] == -1) vis[i] = checkvec[check];
        if (ans[i] == -1)
            range(j, i, far + 1)
                ans[i] = vis[i];
    }

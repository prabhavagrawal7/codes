...
void func()
{
    newint(n, base, height);
    vi vec = inputvec(n);
    vec.push_back(INT_INF);
    ld area = (ld)base * height / 2;
    ld tot = 0;
    range(i, n)
    {
        if (vec[i] == vec[i + 1])
            continue;
        ld exposed = min(height, vec[i + 1] - vec[i]); // exposed height
        // remove triangle
        ld remheight = height - exposed;
        ld rembase = base * remheight / height;
        ld remtri = rembase * remheight / 2;
        tot += area - remtri;
    }

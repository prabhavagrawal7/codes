...
void func()
{
    ...
    {
        lstr = string(rstr.size() - lstr.size(), '0') + lstr;
    }
    ll n = lstr.size();
    string nlstr = lstr;
    range(i, lstr.size())
    {
        if (lstr[i] == rstr[i])
            continue;
        // if (lstr.substr(i + 1, n) == string(n - i - 1, '9'))
        // {
        //     nlstr[i] = rstr[i];
        // }
        range(j, i + 1, lstr.size())
        {
            nlstr[j] = '9';
        }
        break;
    ...
    range(i, lstr.size())
    {
        if (lstr[i] == rstr[i])
            continue;
        // if (rstr.substr(i + 1, n) == string(n - i - 1, '0'))
        // {
        //     nrstr[i] = lstr[i];
        // }
        range(j, i + 1, lstr.size())

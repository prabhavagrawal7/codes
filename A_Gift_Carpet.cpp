...
{
public:
    int rec(int n, int plant_count, string &corridor, vector<vector<int>> &dp)
    {
        if (n == corridor.size())
        {
            if (plant_count < 2)
        ...
        if (dp[n][plant_count] != -1)
            return dp[n][plant_count];
        long ans = 0;
        if (plant_count == 0 || plant_count == 1)
        {
            if (corridor[n] == 'S')
                ans += rec(n + 1, plant_count + 1, corridor, dp);
            else
                ans += rec(n + 1, plant_count, corridor, dp);
        }
        else
        {
            ans += rec(n, 0, corridor, dp);
            if (corridor[n] == 'P')
                ans += rec(n + 1, 2, corridor, dp);
        }
        return dp[n][plant_count] = ans % mod;
    }
...
int main()
{
    ...
    {
        newstring(str);
        cout << Solution().numberOfWays(str) << endl;
    }
}

...
void func()
{
    ...
    foreach (icvec, store)
    {
        auto &cvec = icvec.second;
        cvec.push_front(-1);
        cvec.push_back(n);
        vi planksbtn;
        range(i, cvec.size() - 1)
        {
            planksbtn.push_back(cvec[i + 1] - cvec[i] - 1);
        }
        sort(all(planksbtn), greater<ll>());
        ll planky = planksbtn.front();
        ll maxi = planky / 2;
        ll maxi1 = *max_element(planksbtn.begin() + 1, planksbtn.end());
        ans = min(ans, max(maxi, maxi1));
    }
    print(ans);
}
int main()

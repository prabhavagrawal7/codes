...
void func()
{
    ...
    for (int i = 0; i < k; ++i)
    {
        store[jobs[i].first].push_back({i, 1});
        store[jobs[i].second + 1].push_back({i, -1});
    }

    map<int, int> ansfinder;
    ll ptrAhead = 0, ptrBehind = 0;
    pii brother = {-1, -1}, mother = {INT_INF, -1};
    while (ptrAhead <= n)
    {
        if (ptrAhead - ptrBehind < d)
        {
            auto update = store[ptrAhead];
            for (auto &i : update)
            {
                if (i.second == 1)
                    ansfinder[i.first]++;
        ...
        else if (ptrAhead - ptrBehind == d)
        {
            auto update = store[ptrBehind];
            for (auto &i : update)
            {
                if (i.second == -1)
                    ansfinder[i.first]--;
                if (ansfinder[i.first] == 0)
                    ansfinder.erase(i.first);
            }
            if (brother.first < (ll)ansfinder.size())
                brother = pii(ansfinder.size(), ptrBehind);
            if (mother.first > (ll)ansfinder.size())
                mother = pii(ansfinder.size(), ptrBehind);
            ptrBehind++;
        }
    }
    print(brother.second + 1, mother.second + 1);
}
int main()
{
    FAST;

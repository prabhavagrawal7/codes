...
#endif
/* ----------------------------------------------------------------------------------------------*/
// coordinate compression here

vi kad(vi &vec)
{
    vi ans = {1, vec[0], vec[0]};
    ll now = 1;
    ll l = vec[0], r = vec[0];
    range(i, 1, vec.size())
    {
...
void func()
{
    newint(n);
    vi vec = inputvec(n);
    // store index of each element in map<ll, vi>
    M<ll, vi> m;
    range(i, vec.size()) m[vec[i]].pb(i);
    vi ans(4, -1e9);
    foreach (i, m)
    {
        ans = max(ans, kad(i.second));
    }
    print(vec[ans[1]], ans[1] + 1, ans[2] + 1);
}
int main()
{
    FAST;

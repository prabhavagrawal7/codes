    ...
    while (true)
    {
        ...
        for (int i = 1; i * i <= x; i++)
        {
            if (x % i == 0)
            {
                ...
                if (x / i < store.size() && i * i != x)
                {
                    foreach (j, store[x / i])
                        used[j] -= 1;
                    store[x / i].clear();
                    store[x / i].push_back(x);
                    used[x]++; 
                }
            }
        }

        while (l < used.size() && used[l] == 0)
            l++;
        while (progress < store.size() &&
               store[progress].size() != 0)
            progress++;
        if (progress == store.size())
            diff = min(diff, vec[r] - l);
        r++;
    }
    if (diff == INT_INF)
    {

...
/* ----------------------------------------------------------------------------------------------*/
string str;
vector<ll> back0, back1;
ll tot0 = 0, tot1 = 0;

ll rec(ll n, ll zeros, ll curr)
{
    if (n == str.size())
    {
        if (curr == 0)
    ...
    ll ones = n - zeros;
    auto azeros = tot0 - zeros;
    auto aones = tot1 - ones;
    ll ans = INT_INF;
    if (azeros)
    {
        ll tempans = (str[n] == '0' ? 0 : 1);
        // 01 vale + and 10 vale -
        tempans += rec(n + 1, zeros + 1, curr + aones - ones);
        ans = min(ans, tempans);
    }
    if (aones)
    {
        ll tempans = (str[n] == '1' ? 0 : 1);
        // 01 vale + and 10 vale -
        tempans += rec(n + 1, zeros + 1, curr + zeros - azeros);
        ans = min(ans, tempans);
    }
    return ans;
}
...
{
    cin >> str; 
    tot0 = count(all(str), '0');
    tot1 = count(all(str), '1');
    ll n = str.size();
    back0.assign(n + 1, 0);
    back1.assign(n + 1, 0);
    range(i, n)

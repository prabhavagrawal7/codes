...
#endif
/* ----------------------------------------------------------------------------------------------*/
ll store(pii prevval, pii val)
{
    while (prevval.first < val.first && prevval.second)
    {
        prevval.second--;
        prevval.first <<= 1;
    }
    ll ans = 0;
    while (val.first < prevval.first)
    {
        if (val.second)
            val.first *= 2, val.second--;
        else
            val.first *= 2, ans++;
    }
    return max(0LL, ans + (prevval.second - val.second));
}
void func()
{
    newint(n);

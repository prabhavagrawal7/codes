...
void func()
{
    ...
    range(i, n)
    {
        ...
        {
            ll x = vec[j];
            if (good.empty() || good.back().first <= x)
            {
                good.push_back({x, 1});
            }
            else
            {
                ll lst = good.back().first;
                ll store = 0;
                while (good.size() && good.back().first > x)
                {
                    tans -= good.back().second - 1;
                    store += good.back().second;
                    good.pop_back();
                }
                good.push_back({lst, store + 1});
                tans += store;
            }
            ans += tans;
        }

...
void func()
{
    ...
    si save1;
    range(i, 1, n + 1) save1.insert(i);
    range(i, n) save1.erase(a[i]);

    range(i, n)
    {
        if (a[i] != 0)
            continue;
        auto x = save1.lower_bound(b[i]);
        if (save1.count(b[i]))
            a[i] = b[i];
        else if (x == save1.begin())
        {
            give("NO");
        }
        else
    ...
    si save2;
    range(i, 1, n + 1) save2.insert(i);
    range(i, n) save2.erase(b[i]);
    range(i, n)
    {
        if (b[i] != 0)
            continue;
        auto x = save2.lower_bound(a[i]);
        if (save2.count(a[i]))
            b[i] = a[i];
        else if (x == save2.begin())
        {
            give("NO");
        }
        else

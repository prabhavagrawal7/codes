...
void func()
{
    ...
    range(i, n - 1, -1, -1)
    {
        range(j, 1, sqn)
        {
            if (i + j >= n)
            {
                store[i][j].first = vec[i];
                store[i][j].second = vec[i];
                continue;
            }
            store[i][j].second = store[i + j][j].second + vec[i];
            store[i][j].first = store[i][j].second + store[i + j][j].first;
        }
    }
    vi report;
    range(q)
    {
        ...
        if (d >= sqn)
        {
            ...
            report.push_back(ans);
        }
        else
        {
            ll ans = store[s][d].first;
            if (s + k * d < n)
            {
                ll extraans = store[s + k * d][d].first + store[s + k * d][d].second * k;
                ans -= extraans;
            }
            report.push_back(ans);
        }

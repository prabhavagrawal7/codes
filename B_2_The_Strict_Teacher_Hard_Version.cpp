...
{
    newint(n, m, q);
    vi teachers = inputvec(m);
    vi queries = inputvec(q);
    sort(all(teachers));

    vi ans;
    foreach (query, queries)
    {
        ll nextT = upper_bound(all(teachers), query) - teachers.begin();
        ll prevT = nextT - 1;
        // if teacher is at same position as query
        if (prevT >= 0 && teachers[prevT] == query)
        {
            ans.pb(0);
            continue;
        }
        ...
        if (nextT == m)
        {
            ans.pb(n - teachers[prevT]);
        }
        else if (prevT == -1)
        {
            ans.pb(teachers[nextT] - 1);
        }
        else
        {
            ans.pb((teachers[nextT] - teachers[prevT]) / 2);
        }
    }
    foreach (a, ans)
    {

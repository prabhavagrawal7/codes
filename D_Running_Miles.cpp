...
void func()
{
    ...
    set<pii> parsed, nparsed; 
    range(i, 1, n)
    {
        nparsed.insert({nf[i], i}); 
    }
    parsed.insert({nb[0], 0}); 
    ll ans = 0; 
    range(i, 1, n - 1)
    {
        nparsed.erase({nf[i], i});
        ll u = prev(parsed.end())->second; 
        ll v = prev(nparsed.end())->second; 
        ans = max(ans, (vec[i] + vec[u] + vec[v]) - (v - u)); 
        parsed.insert({nb[i], i}); 
    }
    print(ans); 
}
int main()

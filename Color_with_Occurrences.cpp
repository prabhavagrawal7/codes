...
void func()
{
    ...
    range(i, n)
    {
        if (done[i][0] == 0)
        {
            ...
            range(j, m)
            {
                if (vstr[j] == str.substr(i, vstr[j].size()) &&
                    done[i][1] < i + static_cast<ll>(vstr[j].size()) - 1)
                {
                    noloop = false;
                    done[i] = {1, i + static_cast<ll>(vstr[j].size()) - 1, i, j};
                }
            }
            if (noloop)
            {
            ...
            ans.push_back({done[i][2], done[i][3]});
        }
        else if (done[i][0] == 1)
        {
            range(j, m)
            {
                if (vstr[j] == str.substr(i, vstr[j].size()) &&
                    done[i][1] < i + vstr[j].size() - 1 && done[i][1] + 1 < n)
                {
                    done[done[i][1] + 1] = {2, i + static_cast<ll>(vstr[j].size()) - 1, i, j};
                }
            }
            if (done[i][1] > i && i + 1 < n)
                done[i + 1] = done[i];
        ...
        else
        {
            range(j, m)
            {
                if (vstr[j] == str.substr(i, static_cast<ll>(vstr[j].size())) &&
                    done[i][1] < i + static_cast<ll>(vstr[j].size()) - 1)
                {
                    done[i] = {1, i + static_cast<ll>(vstr[j].size()) - 1, i, j};
                }

...
string check(mii m)
{
    ...
    foreach(i, m)
    {
        newstore = add(newstore, converter(i.second + 1)); 
    }
    foreach(i, newstore)
    {
        if(!m.count(i.first) || m[i.first] < i.second) return "NO"; 
    }
    return "YES"; 
}
void func()

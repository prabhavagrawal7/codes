...
bool check(ll H, ll p, ll M)
{
    ll x = H/p; 
    if(H%p == 0) x--; 
    ll u = M - x*(2*H - p*(x-1))/2; 
    if(u <= 0) return true; 
    else return false; 
}
void func()
{
    ...
    {
        mid = (l + r)/2; 
        ll c1 = check(mid, mp, mh);
        ll c2 = check(mid-1, mp, mh); 
        if(c1 == true && c2 == false)
        {
            give(mid); 
        }
...
int main()
{
    ...
    // srand(time(0));
    // freopen("kingi.txt", "w", stdout);
    // freopen("kingi.txt", "r", stdin);
    // freopen("kingo.txt", "w", stdout);
    // FAST;
    // print(10000);
    newint(t);
    range(t)

...
{
    range(i, b.size()) a.push_back(b[i]);
}
vi subset(vi &a, ll start, ll end)
{
    vi ans;
    range(i, start, end)
    {
...
void func()
{
    newint(n);
    vi vec = inputvec(n);
    ll maxid = max_element(vec.begin() + 1, vec.end()) - vec.begin();
    vi ans;
    range(i, n) ans.push_back(i + 1);

    if (maxid == n - 1)
    {
        range(i, n)
        {
            auto bef = subset(vec, 0, i);
            auto aft = subset(vec, i, n);
            reverse(all(aft));
            vi tempans;
            tempans += aft;
            tempans += bef;
    ...
    range(i, maxid)
    {
        auto fpart = subset(vec, 0, i);
        auto midpart = subset(vec, i, maxid);
        auto endpart = subset(vec, maxid, n);
        reverse(all(midpart));
        vi tempans;
        tempans += endpart;
        tempans += midpart;

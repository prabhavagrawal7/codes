...
#endif
/* ----------------------------------------------------------------------------------------------*/
// check if a bracket sequence is valid or not
...
void func()
{
    newint(n);
    newstring(str);
    // get reverse copy of the str
    if (accumulate(all(str), 0LL, [](ll a, char b)
                   { return a + (b == '(' ? 1 : -1); }) != 0)
        give(-1);
    string rev = str;
    reverse(all(rev));
    ...
    range(i, n)
    {
        ...
        ll extrabal = str[i] == '(' ? 1 : -1;
        if (bal + extrabal < 0 || (extrabal > 0 && prefix[i + 1] < bal + 1))
        {
            ans[i] = 2;
        }
        else

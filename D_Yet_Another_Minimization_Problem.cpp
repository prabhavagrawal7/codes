...
#endif
/* ----------------------------------------------------------------------------------------------*/
ll rec(ll n, vi &a, vi &b, vi &total, ll sum, vvi &dp)
{
    if (n == a.size())
        return 0;
    // state first
    ...
    ll ans1 = sum * a[n] +
              (total[n] - sum) * b[n] +
              rec(n + 1, a, b, total, sum + a[n], dp);
    ll ans2 = sum * b[n] +
              (total[n] - sum) * a[n] +
              rec(n + 1, a, b, total, sum + b[n], dp);
...
void func()
{
    ...
    vvi dp(n, vi(n * 100 + 1, -1));
    ll ans = 2 * rec(0, a, b, tot, 0, dp) + (n - 1) * sqsum(a) + (n - 1) * sqsum(b);
    print(ans);
}
int main()
{

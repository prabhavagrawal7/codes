...
void func()
{
    ...
    // find happiness if one node can be responsible
    ll un = accumulate(all(vec), 0LL);
    range(i, 1, n + 1)
    {
        if (g[i].size() % 2 == 1)
            un = min(un, vec[i - 1]);
    ...
    foreach (i, cvec)
    {
        if (g[i.second].size() % 2 == 0 && g[i.first].size() % 2 == 0)
            un = min(un, vec[i.first - 1] + vec[i.second - 1]);
    }
    give(un);
}
int main()

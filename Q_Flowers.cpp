...
#define rnode (node * 2 + 2)
    ...
    ll rangefind(ll l, ll r, ll node, ll start, ll end)
    {
        ...
        if (start >= l && end <= r)
            return tree[node];
        return max(rangefind(l, r, lnode, start, mid), rangefind(l, r, rnode, mid + 1, end));
    }
};
void func()
{
    newint(n);
    vi height = inputvec(n);
    foreach (i, height)
        --i;
    vi points = inputvec(n);
    map<ll, ll> hpoints;
    range(i, n)
    {
        hpoints[height[i]] = points[i];
    }
    seg tree(n);
    range(i, n - 1, -1, -1)
    {
        ll rempoints = tree.rangefind(height[i], n - 1, 0, 0, n - 1);
        tree.update(height[i], points[i] + rempoints, 0, 0, n - 1);
    }
    print(tree.rangefind(0, n - 1, 0, 0, n - 1));
}
int main()
{
    FAST;

...
#endif
/* ----------------------------------------------------------------------------------------------*/
void find_leafs(ll n, ll par, vi &leafs, vvi &g)
{
    bool isLeaf = true;
    for (ll child : g[n])
    {
...
void func()
{
    ...
    str = 'x' + str;
    vi leafs;
    find_leafs(1, 0, leafs, g);
    ll store[128] = {0}; // array to store character frequencies
    foreach (i, leafs)
        store[str[i]]++;
    ll extraQ = (count(str.begin() + 2, str.end(), '?') - store['?']) % 2;

    if (str[1] != '?')
    {
        give(store[!(str[1] - '0') + '0'] + (store['?'] + 1) / 2);
    }

    if (store['0'] != store['1'])
    {
        ll max_from = store['0'] > store['1'] ? 0 : 1;
        give(store['?'] / 2 + store['0' + max_from]);
    }
    ...
    if (extraQ)
    {
        give(store['0'] + (store['?'] + 1) / 2);
    }
    give(store['0'] + (store['?']) / 2);
}
int main()
{
    FAST;

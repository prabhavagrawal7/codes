...
#define lnode node * 2 + 1
    ...
    void update(ll l, ll r, ll val, ll node, ll start, ll end)
    {
        ...
        {
            tree[node].first = tree[node].second * (end - start + 1);
        ...
        ll left = tree[lnode].second == -1 ? tree[lnode].first : tree[lnode].second * (mid - start + 1);
        ll right = tree[rnode].second == -1 ? tree[rnode].first : tree[rnode].second * (end - (mid + 1) + 1);
        tree[node].first = left + right;
    }
    ...
    ll rfind(ll l, ll r, ll node, ll start, ll end)
    {
        ...
        {
            return tree[node].second == -1 ? tree[node].first : tree[node].second * (end - start + 1);
        }
        if (tree[node].second != -1)
        {
            tree[node].first = tree[node].second * (end - start + 1);

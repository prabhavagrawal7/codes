...
void func()
{
    ...
    range(i, n)
    {
        if(target[i] == str[i]) continue;
        if(target[i] == '0' && _0s.size()) {
            swap(str[_0s.back()], str[i]);
            _0s.pop_back();
            ans++;
        }
        else if(target[i] == '1' && _1s.size()) {
            swap(str[_1s.back()], str[i]);
            _1s.pop_back();
            ans++;
        }
        else if(str[i] == '0') _0s.pb(i);
        else _1s.pb(i);
    }
    print(_0s.size() + _1s.size() + ans); 

...
bool flag;
mii numberline;
void dfs(ll n, V<mii> &g, vi &vis)
{
    vis[n] = true;
    ll ndis = numberline[n];
    foreach (i, g[n])
    {
        ll dis = i.second;
        ll next = i.first;
        if (numberline.count(next) && numberline[next] != ndis + dis)
        {
            flag = true;
            break;
        }
        numberline[next] = ndis + dis;
        if (vis[next])
            continue;
        dfs(next, g, vis);
    }
...
void func()
{
    ...
    {
        newint(a, b, c);
        if (g[a].count(b) && g[a][b] != c)
            flag = true;
        g[a][b] = c;
        if (g[b].count(a) && g[b][a] != -c)
            flag = true;
        g[b][a] = -c;
    }
    if (flag == true)

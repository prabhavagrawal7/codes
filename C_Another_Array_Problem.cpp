...
ll func(vi vec)
{
    if (vec.size() == 2)
    {
        return max(vec[0] + vec[1], 2 * abs(vec[1] - vec[0]));
    }
    else if (vec.size() == 3)
    {
        ll ans = accumulate(all(vec), 0LL);
        ans = max(ans, abs(vec[0] - vec[1]) * 3);
        ans = max(ans, abs(vec[1] - vec[2]) * 3);
        ans = max(ans, abs(vec[0] * 3));
        ans = max(ans, abs(vec[2] * 3));
        return ans;
    }
    else
    {
        return *max_element(all(vec)) * vec.size();
    }
}
int main()
{

...
#endif
/* ----------------------------------------------------------------------------------------------*/
ll timediff(ll from, ll to, ll k)
{
    if (from <= to)
    {
        return to - from;
...
ll dfs(ll n, vvi &bg, vi &time, ll k, vi &dp)
{
    // memo left for now
    if (dp[n] != -1)
        return dp[n];
    ...
    foreach (i, bg[n])
    {
        ans = max(ans, dfs(i, bg, time, k, dp) + timediff(time[i], time[n], k));
    }
    return dp[n] = ans;
}
void func()
{
    ...
    range(i, 1, n + 1)
    {
        ll tottime = dfs(i, bg, time, k, dp);
        ll inittime = ((time[i] - tottime) % k + k) % k;
        store.push_back({inittime, tottime});
    }
    sort(all(store));

    /*
        S, DUR, E
        for ahead:  S + DUR - nowtime
        for before: S + K + DUR - nowtime
    */
    ll ans = INF;
    multiset<ll> back, front;
    foreach (i, store)
    {
        front.insert(i.first + i.second);
    }
    range(i, store.size())
    {
        ll nowtime = store[i].first;
        ll tempans = 0;
        if (front.size())
        {
            auto maxi = prev(front.end());
            tempans = max(tempans, (*maxi - nowtime));
        }
        if (back.size())
        {
            auto maxi = prev(back.end());
            tempans = max(tempans, (*maxi - nowtime));
        }
        // remove from front and add it in the back
        front.erase(front.find(store[i].first + store[i].second));
        back.insert(store[i].first + store[i].second + k);
        ans = min(ans, tempans);
    }
    print(ans);
}

...
void func()
{
    ...
    vvi vis(n, vi(m));
    auto chk = [&n, &m, &vis](ll i, ll j)
    {
        range(u, i - 1, i + 2)
        {
            range(v, j - 1, j + 2)
            {
                if (btn(0LL, u, n - 1) && btn(0LL, v, m - 1) && vis[u][v])
                    return false;
            }
        }
        return true;
    ...
    range(i, n)
    {
        range(j, m)
        {
            if (vstr[i][j] == '*' && !vis[i][j])
            {
                ...
                if (i + 1 < n && j + 1 < m)
                {
                    if (vstr[i][j + 1] == '*' && vstr[i + 1][j] == '*' &&
                        chk(i, j + 1) && chk(i + 1, j))
                        vis[i][j] = 1, vis[i][j + 1] = 1, vis[i + 1][j] = 1;
                    else if (vstr[i + 1][j + 1] == '*' && vstr[i + 1][j] == '*' &&
                             chk(i + 1, j + 1) && chk(i + 1, j))
                        vis[i][j] = 1, vis[i + 1][j + 1] = 1, vis[i + 1][j] = 1;
                    else if (vstr[i][j + 1] == '*' && vstr[i + 1][j + 1] == '*' &&
                             chk(i, j + 1) && chk(i + 1, j + 1))
                        vis[i][j] = 1, vis[i][j + 1] = 1, vis[i + 1][j + 1] = 1;
                }
                if (i + 1 < n && j - 1 >= 0 && !vis[i][j] &&
                    vstr[i + 1][j] == '*' && vstr[i + 1][j - 1] == '*' && chk(i + 1, j) && chk(i + 1, j - 1))

...
void func()
{
    ...
    range(bit, 63 - 1, -1, -1)
    {
        ...
        range(i, n)
        {
            add1 += ((vec[i] ^ (1LL << bit)) & (1LL << bit));
            add2 += ((vec[i]) & (1LL << bit));
        }
        if (add1 > add2)
            contrib.push_back({bit, add1, add2});
        else
            contrib.push_back({bit, 0, add2});
    }

    vi suff(contrib.size() + 1);
    range(i, contrib.size() - 1, -1, -1)
    {
        suff[i] = suff[i + 1] + max(contrib[i][1], contrib[i][2]);
    }
    ...
    range(i, contrib.size())
    {
        auto c = contrib[i];
        if (total + (1LL << c[0]) <= x)
        {
            ans = max(ans, tans + c[2] + suff[i + 1]);
            tans += c[1];
            total += (1LL << c[0]); 
        }
        else

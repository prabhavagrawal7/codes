...
ll dfs(ll n, ll par, vvi &g, vi &sub)
{
    if (sub[n] != -1)
        return sub[n];
    sub[n] = 1;
...
void func()
{
    ...
    if (gp(1) == gp(0))
    {
        ...
        range(i, 1, n + 1)
        {
            g[i].push_back(vec[i] + i);
            g[vec[i] + i].push_back(i);
        }
        vi sub(n + 1, -1); // subtree size
        dfs(1, -1, g, sub);
        ll x = 1;
        ll ans = 0;
        ll pathlength = 0;
        while (x != 0)
        {
            ans += dfs(vec[x] + x, x, g, sub) - 1;
            x = vec[x] + x;
            pathlength++;
        }
        ans += pathlength * (n + 1) + (n - pathlength) * (2 * n + 1);
        print(ans);
    }
    else
    {

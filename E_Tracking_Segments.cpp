...
#endif
/* ----------------------------------------------------------------------------------------------*/
bool checker(ll mid, vi &query, V<pii> &vec, ll n)
{
    vi store(n);
    range(i, mid + 1)
    {
    ...
    {
        prefix[i] = prefix[i - 1] + store[i - 1];
    }
    foreach (i, vec)
    {
        ll tot = i.second - i.first + 1;
        ll _1 = prefix[i.second + 1] - prefix[i.first];
        ll _0 = tot - _1;
        if (_1 > _0)
            return true;
    }
...
void func()
{
    ...
    {
        mid = (l + r) / 2;
        if (checker(mid, queries, vec, n))
            r = mid;
        else
            l = mid;
    }
    if (checker(r, queries, vec, n))
        print(r + 1);
    else
        print(-1);
}

...
vi values;
ll rec(ll n, string &str, bool done, char mode, V<V<V<ll>>> &dp)
{
    if (n == -1)
        return 0;
    if (dp[n][done][mode - '@'] != -1e12)
        return dp[n][done][mode - '@'];
    ll ans = -1e12;
    if (done)
    {
        ans = max(ans, rec(n - 1, str, true, max(mode, str[n]), dp) +
                           (str[n] >= mode ? values[str[n]] : -values[str[n]]));
    }
    else
    {
        ans = max(ans, rec(n - 1, str, false, max(mode, str[n]), dp) +
                           (str[n] >= mode ? values[str[n]] : -values[str[n]]));
        for (char i = 'A'; i < 'F'; i++)
        {
            ans = max(ans, rec(n - 1, str, true, max(mode, i), dp) +
                               (i >= mode ? values[i] : -values[i]));
        }
    }
    return dp[n][done][mode - '@'] = ans;
}
void func()
{
    newstring(str);
    V<V<V<ll>>> dp(str.size() + 1, V<V<ll>>(2, V<ll>(6, -1e12)));
    print(rec(str.size() - 1, str, false, 'A' - 1, dp));
}
int main()
{

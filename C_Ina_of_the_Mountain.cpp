...
void func()
{
    ...
    set<pii> store;
    range(i, n + 1) store.insert({sweep[i], i});
    ll sum = 0;
    range(i, n)
    {
        store.erase(store.find({sweep[i], i}));
        sum += sweep[i];

        auto x = *store.begin();
        store.erase(store.begin());
        if (sum - k >= 0 && abs(sweep[i]) + abs(x.first) > abs(sweep[i] - k) + abs(x.first + k))
        {
            sweep[i] -= k;
            x.first += k;
            sweep[x.second] += k;
        ...
        auto y = *prev(store.end());
        store.erase(prev(store.end()));
        if (abs(sweep[i]) + abs(y.first) > abs(sweep[i] + k) + abs(y.first - k))
        {
            sweep[i] += k;
            y.first -= k;
            sweep[y.second] -= k;

...
void func()
{
    vi ab(2), k(2), q(2);
    cin >> ab >> k >> q;
    V<vi> values = {{1, 1}, {1, -1}, {-1, 1}, {-1, -1}};

    S<vi> possiblek;
    for (auto &&i : values)
    {
        // add i to ab
        vi temp = {ab[0] * i[0], ab[1] * i[1]};
        possiblek.insert(temp + k);
        temp = {ab[1] * i[0], ab[0] * i[1]};
        possiblek.insert(temp + k);
    }
    S<vi> possibleq;
    for (auto &&i : values)
    {
        // add i to ab
        vi temp = {ab[0] * i[0], ab[1] * i[1]};
        possibleq.insert(temp + q);
        temp = {ab[1] * i[0], ab[0] * i[1]};
        possibleq.insert(temp + q);
    }
    // take intersection of possiblek and possibleq

    ll ans = 0; 
    for (auto &&i : possiblek)
    {
        if (possibleq.find(i) != possibleq.end())
        {
            ans++;
        }
    }

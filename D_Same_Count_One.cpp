...
void func()
{
    ...
    range(j, m)
    {
        ...
        range(i, n)
        {
            if (sumall[i] == tsum / n)
                continue;
            if (sumall[i] > tsum / n && vec[i][j] == 1)
            {
                extra.push_back(i); 
            }
            else if(sumall[i] < tsum / n && vec[i][j] == 0)
            {
                lack.push_back(i); 
            }
        }
        while(extra.size() && lack.size())
        {
            ans.push_back({extra.back(), lack.back(), j}); 
            sumall[extra.back()]--; 
            sumall[lack.back()]++; 
            extra.pop_back(), lack.pop_back(); 
        }
    }
    print(ans.size()); 
    foreach(_, ans)

...
#define rnode (node * 2 + 2)
    ...
    seg(ll n)
    {
        ...
        while (n1 < n)
            n1 <<= 1;
        tree.assign(n1 * 2, {0LL, 0LL});
        return;
    }

    void uf(ll l, ll r, ll v, ll node, ll start, ll end)
    {
        if (r < start || end < l)
            return;
        if (l <= start && end <= r)
        ...
        tree[node].second = 0;
        uf(l, r, v, lnode, start, mid);
        uf(l, r, v, rnode, mid + 1, end);
        tree[node].first = ((tree[lnode].first | tree[lnode].second) &
...
void func()
{
    ...
    range(m)
    {
        ...
        {
            newint(b, c, d);
            s.uf(b, c - 1, d, 0, 0, n - 1);
        }
        else
        {
            newint(b, c);
            print(s.rf(b, c - 1, 0, 0, n - 1));
        }
    }
}
int main()

...
ll func()
{
    ...
    newint(n, k);
    vi ans = inputvec(n * n);
    deque<ll> vec(all(ans));
    sort(all(vec));
    vi medians;
    ll ele = ceil(TLD(vec.size() + 1) / TLD(2)) - 1;
    print(vec[ele]);
    /*    while (vec.size() > 0)
    {
        # #
        ...
        sort(all(row));
        ll ele = ceil(TLD(row.size() + 1) / TLD(2)) - 1;
        medians.push_back(vec[ele]);
    }
    print(*min_element(all(medians)));
    */

...
pii func(ll n, ll m, V<vi> &vec)
{
    V<vi> subvec;
    range(i, n)
    {
        subvec.pb(vec[i] - vec[0]);
    }
    V<ll> opr(n);
    bool neverpal = true;
    range(i, 1, subvec.size())
    {
        auto &svec = subvec[i];
        range(i, svec.size())
        {
        ...
        svec = vi(svec.begin() + m - 3, svec.end());
        if (is_pal(svec))
        {
            opr[i] = 0;
            neverpal = false;
            ...
            opr[i] = abs(svec[0] - svec[2]);
    }
    if (neverpal)
    {
        swap(vec[0], vec[1]);
        return {0, func(n, m, vec).second};
    }
    range(i, opr.size())
    {
        if (opr[i] != 0)
...
int main()
{
    ...
    range(t)
    {
        ...
        auto ans = func(n, m, vec);
        print(ans.first + 1, ans.second);
    }
}
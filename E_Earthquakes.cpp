...
struct stree
{
    ...
    {
        this->n = n;
        tree.assign(n * 4, INT_INF);
    }
    void pu(ll ind, ll val, ll node, ll start, ll end)
    {
        if (start == end)
        {
            tree[node] = val;
        ...
        if (ind <= mid)
            pu(ind, val, lnode, start, mid);
        else
            pu(ind, val, rnode, mid + 1, end);
        tree[node] = min(tree[lnode], tree[rnode]);
    }
    ll ru(ll l, ll r, ll val, ll node, ll start, ll end)
    {
        ...
        ll ans = ru(l, r, val, lnode, start, mid) + ru(l, r, val, rnode, mid + 1, end);
        tree[node] = min(tree[lnode], tree[rnode]);
        return ans;
    }
};
void func()
{
    ...
    range(q)
    {
        ...
        {
            newint(ind, val);
            seg.pu(ind, val, 0, 0, n - 1);
        }
        else
        {
            newint(l, r, p);
            print(seg.ru(l, r-1, p, 0, 0, n - 1));
        }
    }
}
int main()

...
#define rnode (node * 2 + 2)
    ...
    {
        this->n = n;
        tree.assign(1LL << (clz(n) + 2), INT_INF);
    }
    void update(ll l, ll r, ll val, ll node, ll start, ll end)
    {
        ...
        if (tree[node] != INT_INF)
        {
            tree[lnode] = min(tree[lnode], tree[node]);
            tree[rnode] = min(tree[rnode], tree[node]);
            tree[node] = INT_INF;
        }
...
void func()
{
    ...
    range(i, 30, -1, -1)
    {
        ...
        range(j, n - 2, -1, -1)
        {
            if (tvec[j] && vec[j] + (j + 1) - 1 >= neartrue)
                neartrue = j;
            else
                tvec[j] = false;
        }

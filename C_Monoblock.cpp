...
void hfunc(vi &ans, ll &tans, ll ind, ll val)
{
    if (ans[ind] == val)
        return;
    ll n = ans.size();
    ll lback = false, lfront = false;
    ll back = false, front = false;

    if (ind - 1 >= 0 && ans[ind] != ans[ind - 1])
        lback = true;
    if (ind + 1 < n && ans[ind] != ans[ind + 1])
        lfront = true;
    if (ind - 1 >= 0 && ans[ind - 1] != val)
        back = true;
    if (ind + 1 < n && ans[ind + 1] != val)
        front = true;

    if (lback == false && back == true)
        tans += ind * (n - ind);
    else if (lback == true && back == false)
        tans -= ind * (n - ind);

    if (lfront == false && front == true)
        tans += (ind + 1) * (n - ind - 1);
    else if (lfront == true && front == false)
        tans -= (ind + 1) * (n - ind - 1);
    ans[ind] = val;
}
void func()
{
    ...
    {
        if (i < n)
            hfunc(ans, tans, i, vec[i]);
        else
        {
            newint(a, b);
            hfunc(ans, tans, a - 1, b);
            print(tans);
        }
    }
}

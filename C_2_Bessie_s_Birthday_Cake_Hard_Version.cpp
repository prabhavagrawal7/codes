...
void func()
{
    ...
    range(i, newvec.size())
    {
        ll val = newvec[i] % n - newvec[(i + 1) % newvec.size()];
        if (val > 0)
            val -= n;
        ll absval = abs(val) - 1;
        if (absval < 2)
            continue;
        extrapoints.push_back(absval);
    }

    sort(all(extrapoints), [](ll a, ll b)
         {
        if(a % 2 == b % 2) return a < b; 
        return a % 2 == 1 && b % 2 == 0; });

    ll points = store.size() - 2;
    range(i, extrapoints.size())
    {
        if (k >= extrapoints[i] / 2)
        {

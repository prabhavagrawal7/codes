...
void func()
{
    int ones = 3, zeros = 1, limit = 2;
    vector<int> onesvec(ones, 1);
    vector<int> zerosvec(zeros, 0);
    vector<int> joined;
    joined.insert(joined.end(), zerosvec.begin(), zerosvec.end());
    joined.insert(joined.end(), onesvec.begin(), onesvec.end());

    // make all possible permutations
    vector<vector<int>> allperms;
    do
    {
        allperms.push_back(joined);
    } while (next_permutation(joined.begin(), joined.end()));
    // check if limit is satisfied i.e. if each subarray size of limit + 1 has atleast one 0 and 1
    // get all subarrays of size limit + 1
    vector<vector<int>> subarrays;
    foreach (perm, allperms)
    {
        // check if each subarray has atleast one 0 and 1
        bool valid = true;
        vector<int> store(2);
        ...
        for (int i = limit + 1; valid && i < perm.size(); i++)
        {
            store[perm[i]]++;
            store[perm[i - limit - 1]]--;
            if (store[0] == 0 || store[1] == 0)
                valid = false;
        }
        if (valid)
        {
            subarrays.push_back(perm);
        }
    }
    // print all valid subarrays
    foreach (subarray, subarrays)
    {
        foreach (elem, subarray)
        {
            cout << elem << " ";
        }
        cout << endl;

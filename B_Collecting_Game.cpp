...
void func()
{
    ...
    range(i, n)
    {
        prefix[i + 1] = prefix[i] + store[i].first;
    }
    vi ans(n);
    range(i, n)
    {
        ll val = prefix[i + 1], it = i;
        while (it < n && prefix[it] <= val)
        {
            ll nit = upper_bound(all(store), mp(val, INT_INF * 1LL)) - store.begin() - 1;
            if (nit == it)
                break;
            else
            {

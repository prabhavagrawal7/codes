...
vvi g, blft;
vi depth, parent;
ll uplift(ll node, ll jump)
{
    if(node == -1 || jump > depth[node]) return -1; 
    range(i, 30, -1, -1)
    {
        if ((jump >> i) & 1)
        {
...
ll lca(ll a, ll b)
{
    if (depth[a] < depth[b])
        swap(a, b);
    a = uplift(a, depth[a] - depth[b]);
    if(a == b) return a; 
    ll l = 0, r = depth[a], mid;
    while (r - l > 1)
    {
        mid = (l + r) / 2;
        if (uplift(a, mid) == uplift(b, mid))
            r = mid;
        else
            l = mid;
    }
...
void func()
{
    ...
    range(i, qn)
    {
        node_depth.push_back({depth[vec[i]], vec[i]});
    }
    sort(all(node_depth));
    vi branches;
    range(i, qn - 1, -1, -1)
    {
        vi tempbranches = branches;
        ll loopcnt = 0;
        foreach (j, branches)
        {
            if (lca(j, node_depth[i].second) == node_depth[i].second)
            {
                j = node_depth[i].second;
                loopcnt++;
            }
        }
        if (loopcnt > 1)
        {
            if (lca(tempbranches[0], tempbranches[1]) == node_depth[i].second && i == 0)
            {
                give("YES");
            }
            else
        ...
        {
            branches.push_back(node_depth[i].second);
        }
        if (branches.size() > 2)
        {
            give("NO");

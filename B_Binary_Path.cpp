...
#endif
/* ----------------------------------------------------------------------------------------------*/
ll rec(ll n, bool state, ll m, V<string> &str, string &smallest)
{
    if (m == smallest.size())
        return 1;
    ll ans = 0;
    if (n != str.size() - 1)
    {
        ans += rec(n + 1, state, m + 1, str, smallest);
    }
    if (state == 0 && str[n][!state] == smallest[m])
    {
        ans += rec(n + 1, true, m + 1, str, smallest); 
    }
    return ans; 
}
void func()
{
    ...
    range(i, str.size())
    {
        ...
        else if (str[i] == '1' && str1[i] == '0')
        {
            ...
            smallest.push_back(str[i]);
    }
    V<string> sstr = {str, str1} ;
    print(smallest);  
    print(rec(0, 0, 0, sstr, smallest)); 
}
int main()
{
    FAST;

...
void func()
{
    ...
    range(i, n)
    {
        if (store.size() == 0 || store.back().first < vec[i])
        {
    ...
    vi keep(n + 1);
    keep[0] = 0;
    keep[store[0].second + 1] = -(cost / store[0].first);
    ll carry = cost % store[0].first;
    ll sum = 0;
    for (int i = 1; i < store.size(); i++)
    {
        ll b = store[i].first;
        sum += store[i - 1].first;
        ll ib = store[i].second, isum = store[i - 1].second;
        if (b - sum > 0)
        {
            ll rem = min(-keep[isum + 1], carry / (b - sum));
            carry = carry - rem * (b - sum);
            keep[isum + 1] += rem, keep[ib + 1] -= rem;
        }
    }
    keep[0] += cost / store[0].first;
    range(i, 1, keep.size())
    {
        keep[i] += keep[i - 1];
    }

...
#endif
/* ----------------------------------------------------------------------------------------------*/
void dfs(ll n, ll par, vvi &g, map<pii, ll> &order, vi &ranked)
{
    foreach (i, g[n])
    {
        if (i == par)
            continue;
        ranked[i] = ranked[n] + 1;
        if (order[{n, par}] < order[{n, i}])
        {
            ranked[i] = ranked[n];
        }
        dfs(i, n, g, order, ranked);
    }
}
void func()
{
    ...
    order[{1, 0}] = 0; 
    vi ranked(n + 1);
    dfs(1, 0, g, order, ranked);
    print(*max_element(all(ranked)) + 1); 
}
int main()
{
    FAST;

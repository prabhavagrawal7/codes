...
/* -------------------------------------------------------------------------------------------------------------------------------------------------------- */
void vtz(string &str, string &str1)
{
    UM<char, char> changes;
    auto initial = 'a';
    foreach (i, str)
    {
        if (changes.count(i))
...
void func()
{
    ...
    while (target != (1 << 11))
    {
        if (popcount(target) > d)
        {
            target++;
            continue;
        }
        bitset<10> ext(target);
        ll tempans = 0;
        ll start = 0, end = INT_INF;
        range(i, n)
        {
            if (ext[str[i] - 'a'] || str[i] == tstr[i])
            {
                if (end == i - 1)
                    end++;
                else
            ...
            {
                if (end != INT_INF)
                    tempans += (end - start + 2) * (end - start + 1) / 2;
                start = 0, end = INT_INF;
            }
        }
        if (end != INT_INF)

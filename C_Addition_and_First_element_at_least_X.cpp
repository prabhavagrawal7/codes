...
public:
    ...
    ll maxind(ll ind, ll val, ll node, ll start, ll end)
    {
        if (end < ind || tree[node] < val)
            return -1;
        if (start == end)
            return start;
        lazy[lnode] += lazy[node];
        ...
        tree[rnode] += lazy[node];
        lazy[node] = 0;
        ll u = maxind(ind, val, lnode, start, mid);
        if (u != -1)
            return u;
        else
            return maxind(ind, val, rnode, mid + 1, end);
    }
};
void func()
{
    ...
    range(i, k)
    {
        ...
        {
            newint(b, c, d);
            s.add(b, c - 1, d, 0, 0, n - 1);
        }
        else
        {
            newint(val, ind);
            print(s.maxind(ind, val, 0, 0, n - 1));
        }
    }
}
int main()

    ...
    range(i, n - 1)
    {
        range(j, m - 1)
        {
            if (vec[i][j] == 1 && vec[i + 1][j + 1] == 1)
                ans = max(ans, goodans - vec[i + 1][j] - vec[i][j + 1]);
            if (vec[i + 1][j] == 1 && vec[i][j + 1] == 1)
                ans = max(ans, goodans - vec[i][j] - vec[i + 1][j + 1]);

            if (vec[i][j] | vec[i + 1][j] | vec[i][j + 1])
                ans = max(ans, goodans + 1 - vec[i][j] - vec[i + 1][j] - vec[i][j + 1]);
            if (vec[i + 1][j + 1] | vec[i + 1][j] | vec[i][j + 1])
                ans = max(ans, goodans + 1 - vec[i + 1][j + 1] - vec[i + 1][j] - vec[i][j + 1]);
            if (vec[i][j] | vec[i + 1][j + 1] | vec[i][j + 1])
                ans = max(ans, goodans + 1 - vec[i][j] - vec[i + 1][j + 1] - vec[i][j + 1]);
            if (vec[i][j] | vec[i + 1][j] | vec[i + 1][j + 1])
                ans = max(ans, goodans + 1 - vec[i][j] - vec[i + 1][j] - vec[i + 1][j + 1]);
        }
    }
    print(ans);
}

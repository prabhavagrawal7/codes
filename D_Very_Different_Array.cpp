...
void func()
{
    ...
    deque<ll> ovec(m);
    range(i, m) cin >> ovec[i];

    sort(all(vec));
    sort(all(ovec));
    vi prefixlow(n + 1), prefixhigh(n + 1);
    range(i, 1, n + 1)
    {
        prefixlow[i] = prefixlow[i - 1] + ovec[i - 1];
    }
    range(i, 1, n + 1)
    {
        prefixhigh[i] = prefixhigh[i - 1] + ovec[m - i];
    }
    ll ans = 0;
    ll rsum = accumulate(all(vec), 0LL);
    ll lsum = 0;
    range(i, vec.size())
    {
        lsum += vec[i];
        ...
        ll left = i;
        ll right = n - left;
        ll val_left = prefixhigh[left] - lsum;
        ll val_right = rsum - prefixlow[right];
        ans = max(ans, val_left + val_right);
    }
    print(ans);
}
int main()

...
struct segtree
{
    ...
    const ll initend = (1 << (clz(100000) + 2));
...
#define neutral_element vi(2, 0)
    ...
    void push(SegNode *&node, ll start, ll end)
    {
        ...
        if (rnode == nullptr)
            rnode = new SegNode();
        // critical
        lnode->val[1] = max(lnode->val[1], node->val[1]);
        rnode->val[1] = max(rnode->val[1], node->val[1]);
        lnode->val[0] = max(lnode->val[0], node->val[1]);
        rnode->val[0] = max(rnode->val[0], node->val[1]);
        node->val[1] = 0;
    }
    ...
    void rangeupdate(ll l, ll r, ll val, SegNode *&node, ll start, ll end)
    {
        ...
        {
            // critical
            node->val[0] = max(node->val[0], val);
            node->val[1] = max(node->val[1], val);
            return;
        }
        push(node, start, end);
        rangeupdate(l, r, val, lnode, start, mid);
        rangeupdate(l, r, val, rnode, mid + 1, end);
        node->val = opr(lnode->val, rnode->val);
    }

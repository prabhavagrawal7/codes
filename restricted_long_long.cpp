...
public:
    ...
    rll() : val(0) {}
    template <typename T>
    rll(T _n) : val((_n % mod + mod) % mod) {}

    // arithmetic operators
    inline rll operator++() { return val++; }
    inline rll operator--() { return val--; }
    inline rll operator+(const rll a) const noexcept { return rll(val + a.val); }
    inline rll operator-(const rll a) const noexcept { return rll(val - a.val); }
    inline rll operator*(const rll a) const noexcept { return rll(a.val * val); }
    inline rll operator/(const rll a) const noexcept { return rll(power(a, mod - 2) * val); }
    inline rll operator%(const rll a) const noexcept { return rll(val % a.val); }
    inline rll operator>>(const rll a) const noexcept { return rll(val >> a.val); }
    inline rll operator<<(const rll a) const noexcept { return rll(val << a.val); }
    template <typename T>
    inline rll operator+(T a) const noexcept { return *this + rll(a); }
    template <typename T>
    inline rll operator-(T a) const noexcept { return *this - rll(a); }
    template <typename T>
    inline rll operator*(T a) const noexcept { return *this * rll(a); }
    ...
    template <typename T>
    inline void operator+=(T a) { val = val + rll(a); }
    template <typename T>
    inline void operator-=(T a) { val = val - rll(a); }
    template <typename T>
    inline void operator*=(T a) { val = val * rll(a); }

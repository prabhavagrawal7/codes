...
/* -------------------------------------------------------------------------------------------------------------------------------------------------------- */
ll code(ll n, vi &vec, vi &dp, map<ll, vi> &toind, vi &pref)
{
    if (n == vec.size())
        return 0;
    if (dp[n] != -1)
    ...
    ll ans = INT_INF;
    ans = min(ans, 1 + code(n + 1, vec, dp, toind, pref));
    if (it != v.end())
    {
        if (*it == n + 1)
            ans = min(ans, code(n + 1, vec, dp, toind, pref));
        else
            ans = min(ans, *it - n - 1 + code(*it, vec, dp, toind, pref));
    }
    return dp[n] = ans;
}
void func()
{
    ...
    for (int i = 0; i < pref.size(); i++)
        toind[pref[i]].pb(i);
    vi dp(n + 1, -1);
    print(code(0, vec, dp, toind, pref));
    // foreach (i, toind)
    // print(i);
}
int main()

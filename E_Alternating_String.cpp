...
#endif
/* ----------------------------------------------------------------------------------------------*/
ll check(string &str, char c1, char c2)
{
}
template <typename T>
inline void operator+=(vector<T> &a, vector<T> &b)
{
    range(i, a.size()) a[i] += b[i];
}
template <typename T>
vector<T> operator-(vector<T> &a, vector<T> &b)
{
    vector<T> c(a.size());
    range(i, a.size()) c[i] = a[i] - b[i];
    return c;
}
template <typename T>
vector<T> operator+(vector<T> &a, vector<T> &b)
...
void func()
{
    newint(n);
    newstring(str);
    vector<vector<vector<int>>> store(n + 1, vector<vector<int>>(2, vector<int>(26)));
    range(i, n)
    {
        store[i + 1] += store[i];
        store[i + 1][i % 2][str[i] - 'a']++;
    }
    ll ans = n + 1;
    if (n % 2 == 1)
    {
        range(i, n)
        {
            vector<vector<int>> ahead =
                {
                    store[n][0] - store[i + 1][0],
                    store[n][1] - store[i + 1][1]};
            auto before = store[i];
            auto val1 = ahead[0] + before[1];
            auto val2 = ahead[1] + before[0];
            ans = min(ans, n - *max_element(all(val1)) - *max_element(all(val2)));
        }
    }
    else
        ans = n - *max_element(all(store[n][0])) - *max_element(all(store[n][1]));
    print(ans);
}
int main()
{

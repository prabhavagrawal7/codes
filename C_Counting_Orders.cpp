...
ll ncr(ll n, ll r)
{
    if (r > n || r < 0)
        return 0;
    return rs(rs(fac(n) * inv(fac(r))) * inv(fac(n - r)));
}
...
void func()
{
    ...
    {
        ll req = itr->second, due = itr->first;
        ll idx = upper_bound(all(a), due) - a.begin();
        if (a.size() - idx < req)
            give(0);
        ll tans = ncr(a.size() - idx, req) * fac(req);
        tans %= mod;
        ans *= tans;
        ans %= mod;
        range(req) a.pop_back();
    }
    print(ans);
}
int main()

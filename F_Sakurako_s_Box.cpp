...
#endif
/* ----------------------------------------------------------------------------------------------*/
ll findProductSum(vi &vec)
{
    ll n = vec.size();
    ll array_sum = 0;
    ll array_sum_square = 0;
    ll individual_square_sum = 0;
    for (ll i = 0; i < n; i++)
    {
        array_sum = (array_sum + vec[i]) % mod;
        individual_square_sum = (individual_square_sum + (ll)vec[i] * vec[i]) % mod;
    }
    array_sum_square = (array_sum * array_sum) % mod;

    ll result = (array_sum_square - individual_square_sum + mod) % mod;

    return (result * ((mod + 1) / 2)) % mod; // Multiply by the modular inverse of 2 under MOD
}
void func()
{
    newint(n);
    vi vec = inputvec(n);
    ll num = findProductSum(vec);
    ll den = inv((n * (n - 1)) / 2);
    print(rs(num * den));
}
int main()

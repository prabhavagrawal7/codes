...
void func()
{
    ...
    while (sum != 1)
    {
        range(i, n - 1)
        {
            if (ans[i] != 1)
                continue;
            ll id1 = i;
            ll id2 = find(ans.begin() + id1 + 1, ans.end(), 1) - ans.begin();
            if (id2 == n)
                break;
            // inversions
            ll iv2 = check(id1, id2);
            ll iv1 = check(id1 + 1, id2);
            // if maxi then count equals
            if (iv2 - iv1 != id2 - id1)
            {
                ans[id1] = 0, sum -= 1;
            }
    ...
    print("!", find(all(ans), 1) - ans.begin() + 1);
}
int main()
{
    FAST;

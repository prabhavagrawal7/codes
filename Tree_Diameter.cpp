...
UM<ll, UM<ll, pii>> dp;
pii func(vector<vector<ll>> &g, ll x, ll parent = -1)
{
    if (dp[x].find(parent) != dp[x].end())
        return dp[x][parent];
    ll ans = 0;
    ll frommax = x;
    for (auto i : g[x])
    ...
    return dp[x][parent] = mp(ans, frommax);
}
int main()
{
    ...
    ll frommax = func(g, 1).second;
    // print(frommax);
    dp.clear();
    print(func(g, frommax).first);
}

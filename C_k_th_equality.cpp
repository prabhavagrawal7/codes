...
{
    ll ans = 0;
    range(x) ans = ans * 10 + 9;
    return ans;
}
ll lowest(ll x)
{
    ll ans = 1;
    range(x - 1) ans = ans * 10;
    return ans;
}
ll dig(ll x)
{
...
ll scrapeDig(ll n, ll dig)
{
    if (n < lowest(dig))
        return 0;
    n = min(n, highest(dig));
    return n - lowest(dig) + 1;
}
ll scrape(ll low, ll high, ll dig)
{
    return scrapeDig(high, dig) - scrapeDig(low-1, dig);
}
...
void func()
{
    ...
    while (k)
    {
        ...
        ll blow = lowest(b);
        ll bhigh = highest(b);
        count = max(count, scrape(A + blow, A + bhigh, c));

        ll clow = lowest(c);
        ll chigh = highest(c);
        count = max(count, scrape(clow - A, chigh - A, b));
        if (k > count)
        {
            k -= count;
            A += 1;
        ...
        if (dig(blow) == b && dig(blow + A) == c)
        {
            report(A, blow + k - 1, A + blow + k - 1);
            return;
        }
        else if (dig(clow - A) == b && dig(clow) == c)
        {
            report(A, clow + k - A - 1, clow + k - 1);
            return;
        }
    }
}

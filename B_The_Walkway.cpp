...
void func()
{
    ...
    {
        ll ahead = prefix.back() - prefix[i + 1];
        ll before = prefix[i];
        ll erem = vec[i] - vec[i - 1];
        ll rem = (erem % k == 0) ? (erem / k - 1) : erem / k;
        before -= rem;
        ll e = vec[i + 1] - vec[i - 1];
        ll extra = (e % k == 0) ? e / k - 1 : e / k;
        ans[ahead + before + extra]++;
    }
    print(ans.begin()->first, ans.begin()->second);
}
int main()
{
    FAST;

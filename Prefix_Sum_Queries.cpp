...
#define rnode (node * 2 + 2)
    ...
    void update(ll x, ll idx, ll node, ll start, ll end)
    {
        if (start == end)
        {
            tree[node][0] = x;
            tree[node][1] = max(0LL, x);
            return;
        }
        if (idx <= mid)
            update(x, idx, lnode, start, mid);
        else
            update(x, idx, rnode, mid + 1, end);
    }
    vi rfind(ll l, ll r, ll node, ll start, ll end)
    {
        if (start > r || end < l)
            return {-1, -1};
        if (start >= l && end <= r)
            return tree[node];
        return opr(rfind(l, r, lnode, start, mid), rfind(l, r, rnode, mid + 1, end));
    }
};
void func()
{
    ...
    range(b)
    {
        ...
        if (a == 1)
        {
            s.update(c, b - 1, 0, 0, a - 1);
        }
        else
        {
            vi ans = s.rfind(b - 1, c - 1, 0, 0, a - 1);
            print(ans[1]);
        }
    }
}

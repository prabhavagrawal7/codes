...
vi closest_find(vi &vec)
{
    ...
    range(i, n)
    {
        ...
        if (i != 0)
        {
            if (disf > abs(vec[i] - vec[i - 1]))
            {
                disf = abs(vec[i] - vec[i - 1]);
                dis = i - 1;
            }
        }
        if (i != n - 1)
...
void func()
{
    newint(n);
    vi vec = inputvec(n);
    // find closest for all vec
    vi closest = closest_find(vec);
    V<vi> forward(n, vi(20, INT_INF));
    V<vi> backward(n, vi(20, INT_INF));
    // processing forward
    range(i, n - 2, -1, -1)
    {
        forward[i][0] = vec[i + 1] - vec[i];
        if (closest[i] == i + 1)
            forward[i][0] = 1;
        range(j, 1, 20)
        {
            if (i + (1 << j) >= n)
                break;
            forward[i][j] = forward[i][j - 1] + forward[i + (1 << (j - 1))][j - 1];
        }
    }
    // processing backward
    range(i, 1, n)
    {
        backward[i][0] = vec[i] - vec[i - 1];
        if (closest[i] == i - 1)
            backward[i][0] = 1;
        range(j, 1, 20)
        {
            if (i - (1 << j) < 0)
                break;
            backward[i][j] = backward[i][j - 1] + backward[i - (1 << (j - 1))][j - 1];
        }
    }

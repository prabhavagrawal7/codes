...
void func()
{
    ...
    pii rs, re;
    cin >> rs.first >> rs.second >> re.first >> re.second;
    rs.first -= re.first, rs.second -= re.second; 
    foreach (x, vec)
    {
        x.first -= re.first;
        x.second -= re.second;
    ...
    ll dis = rs.first * rs.first + rs.second * rs.second;
    foreach (i, vec)
    {
        if (i.first * i.first + i.second * i.second <= dis)
            give("NO");
    }
    give("YES");
}

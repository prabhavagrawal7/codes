...
void func()
{
    ...
    {
        // alice wins
        ll alice_moves = abs(xa - xb) / 2 + 1;
        ll bob_moves = alice_moves - 1;
        // check for draw
        pii bobval = {max(1LL, yb - bob_moves), min(w, yb + bob_moves)};
        pii aliceval = {max(1LL, ya - alice_moves), min(w, ya + alice_moves)};
        if (aliceval.first <= bobval.first && bobval.second <= aliceval.second)
            give("Alice");
        else
            give("Draw");
    }
    ...
    {
        ll alice_moves = abs(xa - xb) / 2;
        ll bob_moves = alice_moves;
        // check for draw
        pii bobval = {max(1LL, yb - bob_moves), min(w, yb + bob_moves)};
        pii aliceval = {max(1LL, ya - alice_moves), min(w, ya + alice_moves)};
        if (bobval.first <= aliceval.first && aliceval.second <= bobval.second)
            give("Bob");
        else
            give("Draw");
    }

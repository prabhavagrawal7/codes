...
void func()
{
    ...
    V<vi> recadd(1001, vi(1001));
    range(i, 1, recadd.size())
    {
        range(j, 1, recadd[i].size())
        {
            recadd[i][j] += recadd[i - 1][j];
            recadd[i][j] += recadd[i][j - 1];
            recadd[i][j] -= recadd[i - 1][j - 1];
            if (rect.count({i, j}))
                recadd[i][j] += rect[{i, j}] * i * j;
        }
    }
    range(i, q)
    {
        newint(hs, ws, hb, wb);
        ll ans = recadd[hb - 1][wb - 1] -
                 (recadd[hb - 1][ws] +
                  recadd[hs][wb - 1] -
                  recadd[hs][ws]);
        print(max(0LL, ans));

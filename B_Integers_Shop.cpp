...
void func()
{
    ...
    range(i, n)
    {
        vec.push_back(inputvec(3));
    }
    V<vi> ans(n, vi(3, -1));
    ll mini = INT_INF, maxi = -INT_INF, cmini = 0, cmaxi = 0;
    range(i, n)
    {
        auto &x = vec[i];
        if (mini > x[0] || (mini == x[0] && cmini > x[2]))
        {
            mini = x[0];
            cmini = x[2];
        }
        if (maxi < x[1] || (maxi == x[1] && cmaxi > x[2]))
        {
            maxi = x[1];
            cmaxi = x[2];
        }
    ...
    vi singleton = {INT_INF, -INT_INF, 0};
    range(i, n)
    {
        if (vec[i][0] <= singleton[0] && vec[i][1] >= singleton[1])
        {
            if (!(vec[i][0] == singleton[0] && vec[i][1] == singleton[1] &&
                  vec[i][2] > singleton[2]))
                singleton = vec[i];
        }
        if (ans[i][0] == singleton[0] && ans[i][1] == singleton[1] && ans[i][2] > singleton[2])
            ans[i] = singleton;
    }
    foreach(i, ans){
        print(i[2]); 

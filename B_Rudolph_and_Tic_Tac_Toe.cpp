...
/* ----------------------------------------------------------------------------------------------*/
// transpose a matrix
V<string> transpose(V<string> vstr)
{
    V<string> ans(vstr[0].size(), string(vstr.size(), ' '));
    range(i, vstr.size())
    {
        range(j, vstr[0].size())
        {
...
void func()
{
    ...
    // check if any row or column have same values
    range(i, 3)
    {
        if (vstr[i][0] == vstr[i][1] && vstr[i][1] == vstr[i][2] && vstr[i][0] != '.')
        {
            give(vstr[i][0]);
        }
        if (vstr[0][i] == vstr[1][i] && vstr[1][i] == vstr[2][i] && vstr[0][i] != '.')
        {
            give(vstr[0][i]);
        }
    }
    // check if any diagonal have same values
    if (vstr[0][0] == vstr[1][1] && vstr[1][1] == vstr[2][2] && vstr[0][0] != '.')
    {
        give(vstr[0][0]);
    }
    if (vstr[0][2] == vstr[1][1] && vstr[1][1] == vstr[2][0] && vstr[0][2] != '.')
    {
        give(vstr[0][2]);
    }
    give("DRAW");

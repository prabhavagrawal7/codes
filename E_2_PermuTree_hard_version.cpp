...
ll minDifference(vi &arr)
{
    ...
    {
        return arr.back() - (sum - arr.back());
    }

    V<pii> store;
    range(i, arr.size())
    {
        if (store.empty() || store.back().first != arr[i])
        {
            store.push_back({arr[i], 1});
        }
        else
        {
            store.back().second++;
    ...
    foreach (s, store)
    {
        vector<int> countleft(possible.size());
        range(i, countleft.size())
        {
            if (possible[i])
                countleft[i] = s.second;
        }
        range(i, possible.size())
        {
            if (possible[i] && countleft[i])
            {
                possible[i + s.first] = true;
                countleft[i + s.first] = max(countleft[i + s.first], countleft[i] - 1);
            }
        }   
    }
    ll ans = INT_INF;

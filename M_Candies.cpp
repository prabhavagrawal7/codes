...
#endif
/* ----------------------------------------------------------------------------------------------*/
vi buildprefix(vi &vec)
{
    vi prefix(vec.size() + 1);
    range(i, vec.size())
    {
        prefix[i + 1] = rs(prefix[i] + vec[i]);
    }
    return prefix;
}
void func()
{
    ...
    vvi dp(n + 1, vi(k + 1));
    dp[n][0] = 1; // base case
    vi prefix = buildprefix(dp[n]);
    range(N, n - 1, -1, -1)
    {
        range(K, k + 1)
        {
            dp[N][K] = 0;
            // range(l, 0, min(vec[N] + 1, K + 1))
            // {
            //     dp[N][K] += dp[N + 1][K - l];
            //     dp[N][K] = rs(dp[N][K]);
            // } 
            // N + 1 element ke liye prefix ban ke ready rahega 
            // joh humari help karega dp[N][K] nikalne me 
            ll lim = min(vec[N], K);
            dp[N][K] += prefix[K + 1] - prefix[K - lim];
            dp[N][K] = rs(dp[N][K]);
        }
        // dp[N]
        // prefix = dp[N]
        prefix = buildprefix(dp[N]);
    }
    print(rs(dp[0][k]));
}
int main()

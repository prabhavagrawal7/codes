...
pii nooffactors(ll n)
{

    ll eans = 0; // Print the number of 2s that divide n
    while (n % 2 == 0)
    {
        eans++;
        n = n / 2;
    ...
    ll oans = 0; // Print the number of 2s that divide n
...
ll func()
{
    ...
    auto [evencount, oddcount] = nooffactors(n);

    if (e >= 0 && o >= 0)
    {
        return evencount * e + oddcount * o;
    }
    else if (e >= 0 && o < 0)
    {
        if (evencount > 0)

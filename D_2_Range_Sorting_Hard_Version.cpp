...
void func()
{
    ...
    vvi good;
    // last element ?
    // element count in segment ?
    // answer because of it
    ll ans = 0;
    range(i, n)
    {
        if (good.size() == 0 || good.back()[0] <= vec[i])
        {
            if (good.size() == 0)
                good.push_back({vec[i], 1, 0});
            else
            {
                good.push_back({vec[i], 1, good.back()[0]});
                ans += good.back()[2];
            }
        }
        else
        {
            ll storeele = 1;
            ll last = good.back()[0];
            while (good.size() && good.back()[0] > vec[i])
            {
                storeele += good.back()[1];
                good.pop_back();
            }
            ll parsed = i + 1;
            ll left = parsed - storeele;

            ll leftans = 0;
            if (good.size())
                leftans = good.back()[2];
            ll ansnow = leftans + (storeele - 1) * left;
            ansnow += (storeele - 1) * (storeele) / 2; 
            ans += ansnow;

            good.push_back({last, storeele, ansnow});
        }
    }
    print(ans);
}

...
#endif
/* ----------------------------------------------------------------------------------------------*/
bool checkifnumberispalindrome(ll n)
{
    ll temp = n;
    ll rev = 0;
    while (temp)
...
void func()
{
    ...
    ll ans = 0;
    foreach (j, palindromeset)
    {
        foreach (i, freq)
        {
            if (freq.count(i.first ^ j))
            {
                ans += (i.second) * (freq[i.first ^ j]);
            }
        }
    }
    ans = ans / 2;
...
int main()
{
    // check all numbers from 0 to 2^15 - 1 are palindrome or not

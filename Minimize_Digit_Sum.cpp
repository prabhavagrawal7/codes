...
vi potentialAnswers(ll n)
{
    ...
    for (ll i = n - 10; i < n + 10; i++)
    {
        ...
        auto a = primefactors(i);
        auto b = populateFactors(a);
        pans.insert(b.begin(), b.end());
    }
    return vi(pans.begin(), pans.end());
}
ll func()
{
    new_int_3(n, l, r);
    vi pans = potentialAnswers(n);
    ll cans = INT_MAX, ans = INT_MAX;
    auto lb = lower_bound(pans.begin(), pans.end(), l) - pans.begin();
    auto ub = upper_bound(pans.begin(), pans.end(), r) - pans.begin();
    
    for(auto i = lb; i != ub; i++)
    {
        if (pans[i] < 2)
            continue;
        ll faltupcans = processor(n, pans[i]);
        if (faltupcans <= cans)
        {
            ans = pans[i];
            cans = faltupcans;

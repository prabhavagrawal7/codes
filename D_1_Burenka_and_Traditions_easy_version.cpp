...
/* -------------------------------------------------------------------------------------------------------------------------------------------------------- */
ll code(ll n, ll curstate, vi &vec, vvi &dp)
{
    if (n == vec.size())
        return 0;
    if (dp[n][curstate] != -1)
        return dp[n][curstate];
    ll ans = INT_INF;
    if (n + 1 < vec.size())
    {
        if (curstate == 0)
            ans = min(ans, code(n + 1, vec[n + 1], vec, dp));
        ans = min(ans, 1 + code(n + 1, vec[n + 1] ^ curstate, vec, dp));
        ans = min(ans, 1 + code(n + 1, vec[n + 1], vec, dp));
    }
    else
    {
        if (curstate == 0)
            ans = min(ans, code(n + 1, vec[n], vec, dp));
        ans = min(ans, 1 + code(n + 1, vec[n] ^ curstate, vec, dp));
        ans = min(ans, 1 + code(n + 1, vec[n], vec, dp));
    }
    return dp[n][curstate] = ans;
}
void func()
{
    newint(n);
    vi vec = inputvec(n);
    V<vi> dp(n + 1, vi(8192, -1));
    give(code(0, vec[0], vec, dp));
}
int main()
{

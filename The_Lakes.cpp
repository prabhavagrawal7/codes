...
/* ----------------------------------------------------------------------------------------------*/
ll n, m;
ll dfs(ll a, ll b, vvi &vec, vvi &vis)
{
    V<pii> parse = {{0, 1}, {0, -1}, {1, 0}, {-1, 0}};
    vis[a][b] = 1;
    ll ans = vec[a][b];
    foreach (xab, parse)
    {
        ll xa = a + xab.first, xb = b + xab.second;
        if (btn(0LL, xa, n - 1) &&
            btn(0LL, xb, m - 1) &&
            vec[xa][xb] != 0 && !vis[xa][xb])
        {
            ans += dfs(xa, xb, vec, vis);
        }
    }
    return ans;
}

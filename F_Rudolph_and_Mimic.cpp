...
#endif
/* ----------------------------------------------------------------------------------------------*/
ll samesame(vi vec, vi newvec)
{
    map<ll, ll> freqvec, freqnewvec;
    range(i, vec.size())
    {
        freqvec[vec[i]]++;
        freqnewvec[newvec[i]]++;
    }
    foreach (i, freqnewvec)
    {
        if (i.second > freqvec[i.first])
            return i.first;
    }
    return -1;
}
void func()
{
    ...
    while (true)
    {
        ...
        else
        {
            ...
            {
                print("- 0");
                vi newvec = inputvec(n);
                ll save = samesame(vec, newvec);
                if (save == -1)
                    continue;
                else
                {

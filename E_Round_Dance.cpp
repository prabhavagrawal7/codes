...
ll gp(ll n, vi &parent)
{
    ...
    else return parent[n] = gp(parent[n], parent); 
}
...
void func()
{
    ...
    vector<set<ll>> g(n); 
    vi parent(n); range(i, n) parent[i] = i; 
    ll straight = 0, round = 0; 
    range(i, n)
    {
        ll x = i, y = vec[i]; 
        ...
        g[x].insert(y); 
        g[y].insert(x); 
        if(parent[gp(x, parent)] == parent[gp(y, parent)]) {
            round += 1; 
        }
        parent[gp(x, parent)] = parent[gp(y, parent)]; 
    }

    map<ll, ll> sector; 
    range(i, n) sector[gp(i, parent)]++; 
    ll total = sector.size(); 
    straight = total - round; 

    // find mini
    ll minans = 0; 
    if(straight == 0) minans = round; 
    else minans = round + 1; 

    // find maxi 
    ll maxans = round + straight; 
    print(minans, maxans); 

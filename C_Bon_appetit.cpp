...
void func()
{
    ...
    MS<ll> svec, sseats(all(seats)); 

    foreach(i, freq) svec.insert(i.second); 

    ll ans = 0; 
    while(svec.size() && sseats.size())
    {
        auto cluster = prev(svec.end());
        auto seat = prev(sseats.end()); 
        ...
        {
            ans += *seat; 
            ll newcluster = *cluster - *seat; 
            sseats.erase(seat); 
            svec.erase(cluster); 
            svec.insert(newcluster); 
        }
    }
    print(ans); 

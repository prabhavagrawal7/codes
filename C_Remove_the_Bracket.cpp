...
/* -------------------------------------------------------------------------------------------------------------------------------------------------------- */
ll rec(ll n, vi &vec, ll removal, ll s, V<vi> &dp)
{
    ll ans = INF;
    ll ele = vec[n - 1];
    if (removal)
        ele = max(0LL, vec[n - 1] - s);
    else
        ele = min(vec[n - 1], s);
    ...
    if (n == 1)
        ele = vec[0];
    ans = min(ans, ele * min(vec[n], s) + rec(n + 1, vec, true, s, dp));
    ans = min(ans, ele * max(0LL, vec[n] - s) + rec(n + 1, vec, false, s, dp));
    return dp[n][removal] = ans;
}
void func()
{
    newint(n, s);

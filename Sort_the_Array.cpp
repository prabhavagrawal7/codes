...
void func()
{
    ...
    map<ll, ll> ind;
    ind[vec[0]] = 0;
    ll last = 0, lastval = vec[0], madefrom = 0;
    range(i, 1, n)
    {
        ll contender = INT_INF;
        if (last == i - 1 && lastval <= vec[i])
            contender = min(contender, madefrom);
        if (ind.count(vec[i]))
            contender = min(contender, ind[vec[i]] + 1);
        if (contender != INT_INF)
        {
            last = i;
            lastval = vec[i];
            madefrom = contender;
            if (ind.count(vec[i]))
                ind[vec[i]] = min(ind[vec[i]], contender);
            else ind[vec[i]] = contender; 
        }
    }
    if (last != n - 1)
    {

...
/* ----------------------------------------------------------------------------------------------*/
vvi dp;
ll rec(ll n, bool erasejump, vi &vec, UM<ll, vi> &indx)
{
    if (n == vec.size())
        return 0;
    if (dp[n][erasejump] != -1)
        return dp[n][erasejump];
    ll ans = 0;
    if (erasejump)
        ans = max(ans, 1 + rec(n + 1, false, vec, indx));
    else
        ans = max(ans, rec(n + 1, false, vec, indx));
    auto &bx = indx[vec[n]];
    auto cx = upper_bound(all(bx), n);
    if (cx != bx.end())
    {
        ans = max(ans, *cx - n + rec(*cx, true, vec, indx));
    }
    return dp[n][erasejump] = ans;
}
void func()
{
    newint(n);

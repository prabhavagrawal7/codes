...
void func()
{
    newint(n);
    vi vec = inputvec(n);
    deque<ll> dvec(all(vec));
    while (dvec.size() && dvec.back() != 0)
    {
        dvec.push_back(dvec.front());
        dvec.pop_front();
    }
    ...
    while (dvec.front() != 0)
    {
        ll newval = dvec.front();
        dvec.pop_front();
        ll count = 0;
        ll remans = 0;
        while (sstg.back()[0] > newval)
        {
            count += sstg.back()[2];
            remans += sstg.back()[2] * sstg.back()[1];
            sstg.pop_back();
        }
        remans += sstg.back()[2] * sstg.back()[1];
        sstg.back()[1] = newval;
        sstg.back()[2] += count;
        tempans = tempans - remans +  xsstg.back()[2] * newval + n;
        ans = max(ans, tempans);
        sstg.push_back({newval, n, 1});
        // foreach(i, sstg) print(i);
    }
    print(ans);
}
int main()

...
void func()
{
    ...
    ll smini = vec[1]; 
    ll maxi = vec.back(); 
    // print(mini, smini, maxi); 
    ll a1 = max(n-1, m-1) * (maxi - mini); 
    ll a2 = min(n-1, m-1) * (maxi - smini); 
    ll a3 = (n - 1) * (m-1) * (maxi - mini); 
    ll ans1 = a1 + a2 + a3; 

    ll smaxi = vec[n * m - 2]; 
    ll b1 = max(n-1, m-1) * (mini - maxi); 
    ll b2 = min(n-1, m-1) * (mini - smaxi); 
    ll b3 = (n - 1) * (m-1) * (mini - maxi); 
    ll ans2 = abs(b1 + b2 + b3); 
    print(max(ans1, ans2)); 
}
int main()
{

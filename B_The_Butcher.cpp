...
bool rec(ll x, ll y, multiset<pii> vecx, multiset<pii> vecy)
{

    while (x != 0 && y != 0)
    {
        pii px = *prev(vecx.end()); 
        pii py = *prev(vecy.end()); 
        if(px.first == x) {
            y -= px.second; 
            vecx.erase(vecx.find(px)); 
            swap(px.first, px.second); 
            vecy.erase(vecy.find(px)); 
        }
        else if(py.first == y){
            x -= py.second; 
            vecy.erase(vecy.find(py)); 
            swap(py.first, py.second); 
            vecx.erase(vecx.find(py)); 
        }
        else return 0; 
    }
    return 1; 
...
void func()
{
    ...
    set<vi> ans; 
    ll u = prev(vecx.end())->first; 
    if(area % u == 0 && rec(u, area/u, vecx, vecy)) {
        ans.insert({u, area/u}); 
    }
    ll v = prev(vecy.end())->first; 
    if(area % v == 0 && rec(area/v, v, vecx, vecy)) {
        ans.insert({area/v, v}); 
    }
    print(ans.size()); 
    foreach(i, ans) {

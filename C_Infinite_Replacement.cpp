...
ll power(ll x, ll y){ll res = 1;while (y){if(y&1LL)res=(res*x);y>>=1;x=(x*x);}return res;}
...
void func()
{
    newstring(str); 
    newstring(str1); 
    if(str1.size() == 1 && str1[0] == 'a') {give(1);}
    if(count(all(str1), 'a')) {give(-1); }
    ll ans = power(2, str.size()); 
    print(ans); 
}
int main()
{

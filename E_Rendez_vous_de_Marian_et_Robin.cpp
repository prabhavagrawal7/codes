...
void func()
{
    ...
    range(i, m)
    {
        ...
        g[opp(a)].pb({opp(b), w / 2});
        g[opp(b)].pb({opp(a), w / 2});
    }
    range(i, 1, n + 1)
    {
        g[opp(i)].pb({i, 0});
    ...
    vi dist = dijkstra(g, 1);
    if (dist[n] == 1e13)
        give(-1);

    vi dist2 = dijkstra(g, n);

    ll ans = 1e13;
    range(i, 1, n + 1)
    {
        ll node = i;
        ll cost = max(dist[node], dist2[node]);
        ans = min(ans, cost);
        node = opp(i);
        cost = max(dist[node], dist2[node]);
        ans = min(ans, cost);
    }
    print(ans);
}
